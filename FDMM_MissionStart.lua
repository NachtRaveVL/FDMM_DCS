...
a(s,s.All)end
local function i(e,i,r)for e,n in pairs(e)do
...
local function t(i,n)local function r(e,n)for e,r in pairs(e)do
if not string.hasAnyPrefix(e,o)then
if not n[e]then
n[e]=r
elseif n[e]~=r then
...
local function l(n,e)if type(e)~='table'then return nil end
...
if e+1<=n and l(r(self.ranges[e+1])-1,self.ranges[e])then
self.ranges[e+1][1]=r(self.ranges[e])table.remove(self.ranges,e)end
if e-1>=1 and l(t(self.ranges[e-1])+1,self.ranges[e])then
...
function FDMMYearRange:addYearRange(o,a)assert(type(o)=='number',"Invalid parameter: startYear")assert(type(a)=='number',"Invalid parameter: endYear")local i={o,a}local e=table.getn(self.ranges)if e==0 or(o<r(self.ranges[1])and a>t(self.ranges[e]))then
self.ranges={i}return
end
...
local n=self:_indexOfRangeAdjToOrCont(a)local s=n and self.ranges[n]or nil
if s and o>t(s)then
table.insert(self.ranges,i)self:_checkCombineNeighborsAt(n+1)return
end
local d=l(o,m)local o=l(a,s)if d and o then
i={r(m),t(s)}self:_collapseRangesFromTo(e,n)table.insert(self.ranges,e,i)self:_checkCombineNeighborsAt(e)return
elseif not d and not o then
if a<r(s)then n=n-1 end
self:_collapseRangesFromTo(e,n)table.insert(self.ranges,e,i)self:_checkCombineNeighborsAt(e)return
else
if d then
self:_collapseRangesFromTo(e+1,n)self.ranges[e][2]=t(i)self:_checkCombineNeighborsAt(e)elseif o then
n=n-self:_collapseRangesFromTo(e,n-1)self.ranges[n][1]=r(i)self:_checkCombineNeighborsAt(n)end
return
end
end
...
local i=nil
for n,e in ipairs(table.sortedKeysList(r))do
assert(type(e)=='number',"Invalid format: startYear")local n=r[e]assert(type(n)~='nil',"Invalid format.")if t==e then return n end
if t<e then return i or n end
i=n
end
return i
end
...
function fdmm.regimentTypes.getObjReportNameInYear(r,n)local e=fdmm.regimentTypes._objForYear(r.ReportNaming,n)if string.isNotEmpty(e)then
local i=fdmm.regimentTypes.getObjNameInYear(r,n)local n=fdmm.regimentTypes.getUnitHullNumberingInYear(r,n)if string.isNotEmpty(i)and string.contains(e,'%N')then
e=e:gsub('%%N',i)end
...
function fdmm.regimentTypes.getUnitTypeInYear(n,e)return fdmm.regimentTypes._objForYear(n.Type,e)end
...
function FDMMCargoRoute:addSpawnLocation(e,r,n)table.insert(self.spawnLocs,{spawnFaction=e,spawnPoint=r,egressPoints=n or{}})end
...
local d={[fdmm.consts.CargoRouteGNPrefix.Land]=fdmm.config.gpCache[fdmm.consts.CargoRouteGNPrefix.Land]or{},[fdmm.consts.CargoRouteGNPrefix.Train]=fdmm.config.gpCache[fdmm.consts.CargoRouteGNPrefix.Train]or{},[fdmm.consts.CargoRouteGNPrefix.Air]=fdmm.config.gpCache[fdmm.consts.CargoRouteGNPrefix.Air]or{},[fdmm.consts.CargoRouteGNPrefix.Sea]=fdmm.config.gpCache[fdmm.consts.CargoRouteGNPrefix.Sea]or{}}function _processGroup(o,e,a)local n=fdmm.utils.removeNumericSuffix(fdmm.utils.removeGroupingPrefix(o))local r=fdmm.territories.all[n]if r~=nil then
local l=fdmm.cargoRoutes[n][a]local c=mist.getGroupRoute(o,false)local d=fdmm.utils.getFaction(string.notEmptyElse(e.units[1].unitName,e.country))local m=a:upperFirst()local u=table.concatedWith(fdmm.consts.CargoRouteGNPrefix,fdmm.consts.CargoRouteWPRoutePrefix[m])local e=nil
...
local t,e,o=fdmm.utils.getGroupingComponentsWithSNC(n.name,e,nil)if t==fdmm.consts.TerritoryWPFacilityPrefix.Airbase and string.isNotEmpty(e)then
...
setmetatable(FDMMArmsPlant,{__index=FDMMFacility,__call=function(e,...)return e.new(...)end})function FDMMArmsPlant.new(e,n,r)local e=setmetatable(FDMMFacility.new(e,n,r),FDMMArmsPlant)return e
end
...
setmetatable(FDMMOilField,{__index=FDMMFacility,__call=function(e,...)return e.new(...)end})function FDMMOilField.new(n,r,e)local e=setmetatable(FDMMFacility.new(n,r,e),FDMMOilField)return e
end
...
setmetatable(FDMMPort,{__index=FDMMFacility,__call=function(e,...)return e.new(...)end})function FDMMPort.new(n,e,r)local e=setmetatable(FDMMFacility.new(n,e,r),FDMMPort)return e
end
