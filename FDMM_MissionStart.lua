...
fdmm.setup.gmtOffset=fdmm.enums.TheaterGMT[env.mission.theatre]elseif fdmm.setup.gmtOffset=="FromServer"then
...
local n=fdmm.config.Config.updatedDCSDetected or
fdmm.config.Config.updatedFDMMDetected
for e,i in pairs(fdmm.setup)do
if i=="OnUpdate"then
fdmm.setup[e]=n
end
end
...
function fdmm.utils.getDCSVersion()assert(_ED_VERSION,"Missing value: _ED_VERSION")return _ED_VERSION:match('^DCS/([%w.]*)')end
function fdmm.utils.getFDMMVersion()return tostring(fdmm.majorVersion).."."..tostring(fdmm.minorVersion).."."..tostring(fdmm.patchVersion)end
...
a(o,o.All)end
local function t(e,t,i)for n,e in pairs(e)do
if not fdmm.unitTypes._isReservedKey(n)then
t[n]=(s[e]or i)..'::'..e
end
end
...
local function r(n,i)local function t(e,n)for e,i in pairs(e)do
if not fdmm.unitTypes._isReservedKey(e)then
if not n[e]then
n[e]=i
elseif n[e]~=i then
...
t(n.NATOReporting,i.NATOReporting)t(n.WTOReporting,i.WTOReporting)t(n.ReportNaming,i.ReportNaming)t(n.ProperNaming,i.ProperNaming)t(n.Nicknaming,i.Nicknaming)end
local function e(e)for i,n in pairs(e.All)do
if not fdmm.unitTypes._isReservedKey(i)then
...
local t,e=db.getYearsLocal(e,i)n[string.trim(i)]={t,e}end
else
n['ALL']={1900,9999}end
...
function fdmm.unitTypes.whichAvailableToCountryIn(e,a,i)local t={}for n,r in pairs(e)do
...
function fdmm.unitTypes.anyAvailableToCountryIn(e,i,n)for t,e in pairs(e)do
...
function fdmm.unitTypes.whichListedUnder(e,i)fdmm.utils.ensureReversedDict(i)local n={}for e,t in pairs(e)do
if not string.hasPrefix(e,'_')then
local r=fdmm.unitTypes.getUnitType(t)if i._rev[r]then
n[e]=t
end
end
end
...
function fdmm.unitTypes.getUnitReportName(e,n,i)local t=e..'Reporting'local e=fdmm.consts.UnitType[t][n]if string.isEmpty(e)then
local r=i or fdmm.unitTypes.getUnitTypeGroup(n)local s=r[t][n]local i,a,o=fdmm.utils.splitTuple(s)i=i or s
...
local function u(e,n)if type(e)~='table'or type(n)~='table'then return nil end
...
if d then
self:_collapseRangesFromTo(e+1,t)self.ranges[e][2]=i(r)self:_checkCombineNeighborsAt(e)elseif o then
...
local d={a,r}local t=self:_indexOfRangeAdjToOrCont(a)local m=t and self.ranges[t]or nil
local e=self:_indexOfRangeAdjToOrCont(r)local l=e and self.ranges[e]or nil
local c=s(a,m)local u=s(r,l)local o=a<=n(m)local s=r<n(l)local n=r>=i(l)if o and(s or n)then
if s then e=e-1 end
self:_collapseRangesFromTo(t,e)return
elseif t==e and c and u then
d={r+1,i(m)}self.ranges[t][2]=a-1
table.insert(self.ranges,t+1,d)self:_checkValidRangeAt(t)self:_checkValidRangeAt(t+1)return
else
local i,s=t,e
if not o then i=t+1 end
if not n then s=e-1 end
e=e-self:_collapseRangesFromTo(i,s)if not o and c then
self.ranges[t][2]=a-1
self:_checkValidRangeAt(t)elseif not n and u then
self.ranges[e][1]=r+1
self:_checkValidRangeAt(e)end
return
end
...
function fdmm.regimentTypes.dumpRegimentYearlyAvailability(o,a,r)env.info("FDMM: Dumping regiment yearly availability for years "..tostring(a).." to "..tostring(r).."...")local function t(i,e,n)env.info("FDMM: "..n.."Regiment: "..fdmm.regimentTypes.getObjReportNameIn(i,e))for t,i in pairs(i.Units or{})do
...
setmetatable(FDMMResourceUnit,{__call=function(e,...)return e.new(...)end,})function FDMMResourceUnit.new(o,a,t,i,n,r)local e=setmetatable({},FDMMResourceUnit)e.fuelRP=o
e.armsRP=a
e.unitRP=t
e.maxFuelRP=i
e.maxArmsRP=n
...
local d={[fdmm.consts.CargoRouteGNPrefix.Land]=fdmm.config.gpCache[fdmm.consts.CargoRouteGNPrefix.Land]or{},[fdmm.consts.CargoRouteGNPrefix.Train]=fdmm.config.gpCache[fdmm.consts.CargoRouteGNPrefix.Train]or{},[fdmm.consts.CargoRouteGNPrefix.Air]=fdmm.config.gpCache[fdmm.consts.CargoRouteGNPrefix.Air]or{},[fdmm.consts.CargoRouteGNPrefix.Sea]=fdmm.config.gpCache[fdmm.consts.CargoRouteGNPrefix.Sea]or{}}function _processGroup(o,n,a)local e=fdmm.utils.removeNumericSuffix(fdmm.utils.removeGroupingPrefix(o))local i=fdmm.territories.all[e]if i~=nil then
...
local d,t,i=fdmm.utils.getGroupingComponentsWithSNC(e.name,u,fdmm.consts.CargoRouteWPRouteSuffix)if d==fdmm.consts.CargoRouteGNPrefix[m]then
...
for n,e in pairs(d[fdmm.consts.CargoRouteGNPrefix.Sea])do
...
if e>1 then
local e=mist.utils.makeVec3(a)local e=mist.vec.sub(e,r)local i=mist.vec.mag(e)local a=mist.vec.scalarMult(e,1./i)local e=1
local n=i/e
while n>o do
e=e+1
n=i/e
...
while e>=0 do
local n=mist.vec.scalarMult(a,e*n)local n=mist.vec.add(r,n);COORDINATE:NewFromVec2(mist.utils.makeVec2(n)):Smoke(t)e=e-1
end
end
...
setmetatable(FDMMFacility,{__call=function(e,...)return e.new(...)end})function FDMMFacility.new(i,n,t)local e=setmetatable({},FDMMFacility)e.name=i
e.centerPoint=n
e.territoryName=t
return e
end
...
setmetatable(FDMMCommandCenter,{__index=FDMMFacility,__call=function(e,...)return e.new(...)end})function FDMMCommandCenter.new(n,e,i)local e=setmetatable(FDMMFacility.new(n,e,i),FDMMCommandCenter)return e
end
...
fdmm.unitTypes.createUnitTypeAvailability()fdmm.unitTypes.saveUnitTypeAvailability()else
fdmm.unitTypes.loadUnitTypeAvailability()end
