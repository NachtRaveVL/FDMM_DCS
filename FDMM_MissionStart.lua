...
fdmm.config.gpCache[e][n][r]=i
else
if not fdmm.config.gpCache[e]['_']then
fdmm.config.gpCache[e]['_']={}end
fdmm.config.gpCache[e]['_'][r]=i
end
else
fdmm.config.gpCache[e][r]=i
end
end
end
end
...
local r=string.find(e,'_')if r~=nil then
local e=e:sub(1,r)if e==e:upper()then
return e
end
end
end
...
function fdmm.utils.removeGroupingPrefix(e)local r=fdmm.utils.getGroupingPrefix(e)if r~=nil then
return e:sub(#r+1),r
end
return e,nil
end
function fdmm.utils.getNumericSuffix(e)if e then
local n,r=e:reverse():find('# ')if r~=nil then
local e=e:sub(-r)if e==e:match(' #[%d]+')then
return e
end
end
end
...
function fdmm.utils.removeNumericSuffix(e)local r=fdmm.utils.getNumericSuffix(e)if r~=nil then
return e:sub(1,-#r-1),r
end
return e,nil
end
function fdmm.utils.getGroupingSuffix(e)if e then
local r=e:reverse():find('_')if r~=nil then
return e:sub(-r)end
end
return nil
end
function fdmm.utils.removeGroupingSuffix(r)local e=fdmm.utils.getGroupingSuffix(r)if e~=nil then
return r:sub(1,-#e-1),e
end
return r,nil
end
...
function fdmm.utils.getGroupingComponentsWithSNC(e,i,t)local n,e,r=fdmm.utils.getGroupingComponents(e)if string.isEmpty(n)and table.contains(i or{},(e..'_'))then
n=(e..'_')e=nil
...
function fdmm.utils.reversedDict(e)assert(e,'Nonnull parameter: tbl')local r={}for n,e in pairs(e)do
if(type(n)=='string'or type(n)=='number')and(type(e)=='string'or type(e)=='number')then
if not r[e]then
r[e]=n
elseif r[e]~=n then
if type(r[e])~='table'then
r[e]={r[e]}end
if not table.contains(r[e],n)then
...
local r=string.occurrences(e,':')if r==0 then
return e
elseif r==1 then
return fdmm.utils.splitDoublet(e)elseif r==2 then
return fdmm.utils.splitTriplet(e)elseif r==3 then
...
function string.contains(r,e)return string.find(r,e,0,true)~=nil
end
function string.occurrences(e,r)return select(2,e:gsub(r,''))end
function string.hasPrefix(r,e)return string.sub(r,1,#e)==e
end
function string.hasAnyPrefix(e,r)for n,r in pairs(r)do
if string.hasPrefix(e,r)then
return true
end
end
return false
end
function string.hasSuffix(r,e)return string.sub(r,-#e)==e
end
function string.hasAnySuffix(r,e)for n,e in pairs(e)do
if string.hasSuffix(r,e)then
return true
end
end
return false
...
function string.notEmptyElse(e,r)if e~=nil and#e>0 then
return e
end
return r
end
function string.upperFirst(e)return e:sub(1,1):upper()..e:sub(2)end
function table.contains(e,r)for n,e in pairs(e)do
if e==r then
return true
end
end
...
function table.concatWith(n,e)if e then
...
function table.concatWithi(r,e)if e then
for n,e in ipairs(e)do
table.insert(r,e)end
end
end
function table.keysList(r)local e={}for r,n in pairs(r)do
table.insert(e,r)end
return e
end
...
e._rev[i]=r
elseif e._rev[i]~=r then
env.error("Group all list reverse discrepancy for unitType='"..i.."', groupAllList._rev[unitType]='"..e._rev[i].."' ~= fdmmUnitType='"..r.."'.")end
end
if string.isNotEmpty(t)then
if not e._revShape then e._revShape={}end
if not e._revShape[t]then
e._revShape[t]=r
elseif e._revShape[t]~=r then
env.error("Group all list shape reverse discrepancy for shapeName='"..t.."', groupAllList._revShape[shapeName]='"..e._revShape[t].."' ~= fdmmUnitType='"..r.."'.")end
end
elseif e[r]~=n then
env.error("Group all list discrepancy for fdmmUnitType='"..r.."', groupAllList[fdmmUnitType]='"..e[r].."' ~= value='"..n.."'.")end
end
end
end
end
s(a,a.All)end
local function a(e,n,i)for r,e in pairs(e)do
if not string.hasAnyPrefix(r,o)then
n[r]=(m[e]or i)..'::'..e
end
end
end
local function s(e,n)for e,i in pairs(e)do
if not string.hasAnyPrefix(e,o)then
local t,t,r=fdmm.utils.splitTuple(i)r=r or i
if not n[e]then
n[e]=r
elseif n[e]~=r then
env.error("Master all list discrepancy for fdmmUnitType='"..e.."', masterAllList[fdmmUnitType]='"..n[e].."' ~= unitType='"..r.."'.")end
end
end
end
local function r(e)if not e.NATOReporting then e.NATOReporting={}end
...
local function i(n,r)local function i(e,r)for e,n in pairs(e)do
if not string.hasAnyPrefix(e,o)then
if not r[e]then
r[e]=n
elseif r[e]~=n then
...
local function e(r)for e,n in pairs(r.All)do
if not string.hasAnyPrefix(e,o)then
local i,i,e=fdmm.utils.splitTuple(n)e=e or n
if not r.ReportNaming[e]then
r.ReportNaming[e]=e
end
end
end
end
t(fdmm.consts.UnitType.Plane)t(fdmm.consts.UnitType.Helicopter)t(fdmm.consts.UnitType.Ground)t(fdmm.consts.UnitType.Ship)t(fdmm.consts.UnitType.Train)a(fdmm.consts.UnitType.Plane.All,fdmm.consts.UnitType.Static.Plane,'Planes')a(fdmm.consts.UnitType.Helicopter.All,fdmm.consts.UnitType.Static.Helicopter,'Helicopters')a(fdmm.consts.UnitType.Ground.All,fdmm.consts.UnitType.Static.Ground,'Unarmed')a(fdmm.consts.UnitType.Train.All,fdmm.consts.UnitType.Static.Train,'Unarmed')a(fdmm.consts.UnitType.Ship.All,fdmm.consts.UnitType.Static.Ship,'Ships')t(fdmm.consts.UnitType.Static)table.concatWith(fdmm.consts.UnitType.All,fdmm.consts.UnitType.Plane.All)table.concatWith(fdmm.consts.UnitType.All,fdmm.consts.UnitType.Helicopter.All)table.concatWith(fdmm.consts.UnitType.All,fdmm.consts.UnitType.Ground.All)table.concatWith(fdmm.consts.UnitType.All,fdmm.consts.UnitType.Train.All)table.concatWith(fdmm.consts.UnitType.All,fdmm.consts.UnitType.Ship.All)s(fdmm.consts.UnitType.Static.All,fdmm.consts.UnitType.All)fdmm.consts.UnitType.All._rev={}table.concatWith(fdmm.consts.UnitType.All._rev,fdmm.consts.UnitType.Plane.All._rev)table.concatWith(fdmm.consts.UnitType.All._rev,fdmm.consts.UnitType.Helicopter.All._rev)table.concatWith(fdmm.consts.UnitType.All._rev,fdmm.consts.UnitType.Ground.All._rev)table.concatWith(fdmm.consts.UnitType.All._rev,fdmm.consts.UnitType.Train.All._rev)table.concatWith(fdmm.consts.UnitType.All._rev,fdmm.consts.UnitType.Ship.All._rev)table.concatWith(fdmm.consts.UnitType.All._rev,fdmm.consts.UnitType.Static.All._rev)fdmm.consts.UnitType.Plane.CarrierBorne._rev=fdmm.utils.reversedDict(fdmm.consts.UnitType.Plane.CarrierBorne)fdmm.consts.UnitType.Plane.PlayerControllable._rev=fdmm.utils.reversedDict(fdmm.consts.UnitType.Plane.PlayerControllable)fdmm.consts.UnitType.Helicopter.CarrierBorne._rev=fdmm.utils.reversedDict(fdmm.consts.UnitType.Helicopter.CarrierBorne)fdmm.consts.UnitType.Helicopter.PlayerControllable._rev=fdmm.utils.reversedDict(fdmm.consts.UnitType.Helicopter.PlayerControllable)fdmm.consts.UnitType.Ground.Amphibious._rev=fdmm.utils.reversedDict(fdmm.consts.UnitType.Ground.Amphibious)fdmm.consts.UnitType.Ground.HeavyWheeled._rev=fdmm.utils.reversedDict(fdmm.consts.UnitType.Ground.HeavyWheeled)r(fdmm.consts.UnitType.Plane)r(fdmm.consts.UnitType.Helicopter)r(fdmm.consts.UnitType.Ground)r(fdmm.consts.UnitType.Ship)r(fdmm.consts.UnitType.Train)r(fdmm.consts.UnitType.Static)e(fdmm.consts.UnitType.Plane)e(fdmm.consts.UnitType.Helicopter)e(fdmm.consts.UnitType.Ground)e(fdmm.consts.UnitType.Ship)e(fdmm.consts.UnitType.Train)i(fdmm.consts.UnitType.Plane,fdmm.consts.UnitType.Static)i(fdmm.consts.UnitType.Helicopter,fdmm.consts.UnitType.Static)i(fdmm.consts.UnitType.Ground,fdmm.consts.UnitType.Static)i(fdmm.consts.UnitType.Ship,fdmm.consts.UnitType.Static)i(fdmm.consts.UnitType.Train,fdmm.consts.UnitType.Static)e(fdmm.consts.UnitType.Static)table.concatWith(fdmm.consts.UnitType.Nicknaming,fdmm.consts.UnitType.Plane.Nicknaming)table.concatWith(fdmm.consts.UnitType.Nicknaming,fdmm.consts.UnitType.Helicopter.Nicknaming)table.concatWith(fdmm.consts.UnitType.Nicknaming,fdmm.consts.UnitType.Ground.Nicknaming)table.concatWith(fdmm.consts.UnitType.Nicknaming,fdmm.consts.UnitType.Train.Nicknaming)table.concatWith(fdmm.consts.UnitType.Nicknaming,fdmm.consts.UnitType.Ship.Nicknaming)table.concatWith(fdmm.consts.UnitType.Nicknaming,fdmm.consts.UnitType.Static.Nicknaming)end
...
function fdmm.unitTypes.getUnitReportName(e,r,n)local t=e..'Reporting'local e=fdmm.consts.UnitType[t][r]if string.isEmpty(e)then
local i=n or fdmm.unitTypes.getUnitTypeGroup(r)local s=i[t][r]local n,a,o=fdmm.utils.splitTuple(s)n=n or s
e=i.ReportNaming[r]local i=i.ProperNaming[r]if string.isNotEmpty(n)and string.contains(e,'%R')then
e=e:gsub('%%R',n)end
...
fdmm.consts.UnitType[t][r]=e
end
return e
end
function fdmm.unitTypes.getNATOUnitReportName(e,r)return fdmm.unitTypes.getUnitReportName(fdmm.enums.Alliance.NATO,e,r)end
function fdmm.unitTypes.getWTOUnitReportName(e,r)return fdmm.unitTypes.getUnitReportName(fdmm.enums.Alliance.WTO,e,r)end
function fdmm.unitTypes.dumpUnitReportNames()local function t(r)for e,n in pairs(table.sortedKeysList(r.All))do
if not string.hasPrefix(n,'_')then
local i=r.All[n]local t,t,e=fdmm.utils.splitTuple(i)e=e or i
env.info("  ['"..n.."'] = '"..e.."':")env.info('    NATO: '..fdmm.unitTypes.getNATOUnitReportName(e,r))env.info('    WTO : '..fdmm.unitTypes.getWTOUnitReportName(e,r))end
end
end
...
mist.DBs.Category={Vehicle='vehicle',Ship='ship',Plane='plane',Helicopter='helicopter',Building='building',Unknown='unknown'}function mist.utils.get2DDistSqrd(e,r)if e.z~=nil then e=mist.utils.makeVec2(e)end
if r.z~=nil then r=mist.utils.makeVec2(r)end
return mist.vec.magSqrd({x=e.x-r.x,y=e.y-r.y})end
function mist.utils.get3DDistSqrd(r,e)if r.z==nil then r=mist.utils.makeVec3(r)end
if e.z==nil then e=mist.utils.makeVec3(e)end
return mist.vec.magSqrd({x=r.x-e.x,y=r.y-e.y,z=r.z-e.z})end
...
function routines.utils.get2DDistSqrd(r,e)if r.z~=nil then r=routines.utils.makeVec2(r)end
if e.z~=nil then e=routines.utils.makeVec2(e)end
return routines.vec.magSqrd({x=r.x-e.x,y=r.y-e.y})end
function routines.utils.get3DDistSqrd(e,r)if e.z==nil then e=routines.utils.makeVec3(e)end
if r.z==nil then r=routines.utils.makeVec3(r)end
return routines.vec.magSqrd({x=e.x-r.x,y=e.y-r.y,z=e.z-r.z})end
routines.vec.magSqrd=function(e)return(e.x*e.x)+(e.y*e.y)+((e.z or 0)*(e.z or 0))end
function ZONE_POLYGON:NewFromPoints(r,e)local e=BASE:Inherit(self,ZONE_POLYGON_BASE:New(r,e))e:F({r,nil,e._.Polygon})_EVENTDISPATCHER:CreateEventNewZone(e)return e
end
end
...
setmetatable(FDMMResourceUnit,{__call=function(e,...)return e.new(...)end,})function FDMMResourceUnit.new(t,i,a,o,n,r)local e=setmetatable({},FDMMResourceUnit)e.fuelRP=t
e.armsRP=i
e.unitRP=a
e.maxFuelRP=o
e.maxArmsRP=n
...
setmetatable(FDMMCargoRoute,{__call=function(e,...)return e.new(...)end,})function FDMMCargoRoute.new(r,n)local e=setmetatable({},FDMMCargoRoute)e.territoryName=r
e.routeType=n
e.spawnLocs={}e.facilityLocs={}e.dirLinks={}return e
end
function FDMMCargoRoute:addSpawnLocation(n,r,e)table.insert(self.spawnLocs,{spawnFaction=n,spawnPoint=r,egressPoints=e or{}})end
function FDMMCargoRoute:addFacilityLocation(e,n,r)table.insert(self.facilityLocs,{facilityName=e,facilityPoint=n,ingressPoints=r or{}})end
...
local d={[fdmm.consts.CargoRouteGNPrefix.Land]=fdmm.config.gpCache[fdmm.consts.CargoRouteGNPrefix.Land]or{},[fdmm.consts.CargoRouteGNPrefix.Train]=fdmm.config.gpCache[fdmm.consts.CargoRouteGNPrefix.Train]or{},[fdmm.consts.CargoRouteGNPrefix.Air]=fdmm.config.gpCache[fdmm.consts.CargoRouteGNPrefix.Air]or{},[fdmm.consts.CargoRouteGNPrefix.Sea]=fdmm.config.gpCache[fdmm.consts.CargoRouteGNPrefix.Sea]or{}}function _processGroup(a,e,o)local r=fdmm.utils.removeNumericSuffix(fdmm.utils.removeGroupingPrefix(a))local n=fdmm.territories.all[r]if n~=nil then
local m=fdmm.cargoRoutes[r][o]local u=mist.getGroupRoute(a,false)local d=fdmm.utils.getFaction(string.notEmptyElse(e.units[1].unitName,e.country))local l=o:upperFirst()local c=table.concatedWith(fdmm.consts.CargoRouteGNPrefix,fdmm.consts.CargoRouteWPRoutePrefix[l])local r=nil
local e={Spawn='spawn',Linkage='linkage',Facility='facility',TBD='_'}local n,t,s=nil,nil,{}function _updateScanner(i,l)if r==e.TBD and i~=nil and i~=e.TBD then
r=i
elseif r~=nil and r~=e.TBD and i==e.TBD then
i=r
end
if r~=i or n~=l then
if r then
...
env.error('Cargo '..o.." routing group '"..a.."' failed to parse spawn route '"..(n or'<nil>').."'.")end
elseif r==e.Linkage then
if string.isNotEmpty(n)and t and s then
m:addDirectionalLink(n,t,s)else
env.error('Cargo '..o.." routing group '"..a.."' failed to parse linkage route '"..(n or'<nil>').."'.")end
elseif r==e.Facility then
if string.isNotEmpty(n)and t and s then
m:addFacilityLocation(n,t,s)else
env.error('Cargo '..o.." routing group '"..a.."' failed to parse facility route '"..(n or'<nil>').."'.")end
elseif r~=e.TBD then
...
for m,r in ipairs(u)do
if string.isNotEmpty(r.name)and(r.name..'_')~=fdmm.consts.CargoRouteGNPrefix[l]then
local d,i,n=fdmm.utils.getGroupingComponentsWithSNC(r.name,c,fdmm.consts.CargoRouteWPRouteSuffix)if d==fdmm.consts.CargoRouteGNPrefix[l]then
if n==fdmm.consts.CargoRouteWPRouteSuffix.Egress or
n==fdmm.consts.CargoRouteWPRouteSuffix.Ingress then
_updateScanner(e.TBD,i)table.insert(s,fdmm.utils.rposFromRPoint(r))elseif n==fdmm.consts.CargoRouteWPRouteSuffix.Spawn then
_updateScanner(e.Spawn,i)t=fdmm.utils.rposFromRPoint(r)elseif n==fdmm.consts.CargoRouteWPRouteSuffix.Linkage then
_updateScanner(e.Linkage,i)t=fdmm.utils.rposFromRPoint(r)elseif n==fdmm.consts.CargoRouteWPRouteSuffix.Facility then
_updateScanner(e.Facility,i)t=fdmm.utils.rposFromRPoint(r)elseif not(string.isEmpty(i)and string.isEmpty(n))then
env.error('Cargo '..o.." routing group '"..a.."' unknown WP '"..(r.name or'<nil>').."' at WP index "..m..'.')end
elseif d==fdmm.consts.CargoRouteWPRoutePrefix[l].Spawn then
if string.isEmpty(n)then
_updateScanner(e.Spawn,i)t=fdmm.utils.rposFromRPoint(r)elseif n==fdmm.consts.CargoRouteWPRouteSuffix.Egress then
_updateScanner(e.Spawn,i)table.insert(s,fdmm.utils.rposFromRPoint(r))elseif not(string.isEmpty(i)and string.isEmpty(n))then
env.error('Cargo '..o.." routing group '"..a.."' unknown spawn WP '"..(r.name or'<nil>').."' at WP index "..m..'.')end
elseif d==fdmm.consts.CargoRouteWPRoutePrefix[l].Linkage then
if string.isEmpty(n)then
_updateScanner(e.Linkage,i)t=fdmm.utils.rposFromRPoint(r)elseif n==fdmm.consts.CargoRouteWPRouteSuffix.Ingress then
_updateScanner(e.Linkage,i)table.insert(s,fdmm.utils.rposFromRPoint(r))elseif not(string.isEmpty(i)and string.isEmpty(n))then
env.error('Cargo '..o.." routing group '"..a.."' unknown linkage WP '"..(r.name or'<nil>').."' at WP index "..m..'.')end
elseif d==fdmm.consts.CargoRouteWPRoutePrefix[l].Facility then
if string.isEmpty(n)then
_updateScanner(e.Facility,i)t=fdmm.utils.rposFromRPoint(r)elseif n==fdmm.consts.CargoRouteWPRouteSuffix.Ingress then
_updateScanner(e.Facility,i)table.insert(s,fdmm.utils.rposFromRPoint(r))elseif not(string.isEmpty(i)and string.isEmpty(n))then
env.error('Cargo '..o.." routing group '"..a.."' unknown facility WP '"..(r.name or'<nil>').."' at WP index "..m..'.')end
else
env.error('Cargo '..o.." routing group '"..a.."' unknown routing WP '"..(r.name or'<nil>').."' at WP index "..m..'.')end
end
end
_updateScanner(nil,nil)else
env.error('Cargo '..o.." routing group '"..a.."' failed to find territory with same name.")end
end
for e,r in pairs(d[fdmm.consts.CargoRouteGNPrefix.Land])do
_processGroup(e,r,fdmm.enums.CargoRouteType.Land)end
for e,r in pairs(d[fdmm.consts.CargoRouteGNPrefix.Train])do
_processGroup(e,r,fdmm.enums.CargoRouteType.Train)end
for r,e in pairs(d[fdmm.consts.CargoRouteGNPrefix.Air])do
_processGroup(r,e,fdmm.enums.CargoRouteType.Air)end
for r,e in pairs(d[fdmm.consts.CargoRouteGNPrefix.Sea])do
_processGroup(r,e,fdmm.enums.CargoRouteType.Sea)end
end
end
...
setmetatable(FDMMTerritory,{__call=function(e,...)return e.newFromGroup(...)end,})function FDMMTerritory.newFromGroup(r,n)local e=setmetatable({},FDMMTerritory)e.name=fdmm.utils.removeGroupingPrefix(r)if n.category==mist.DBs.Category.Ship then
e.type=fdmm.enums.TerritoryType.Sea
else
e.type=fdmm.enums.TerritoryType.Land
end
e.polygonPoints={}local i=mist.getGroupPoints(r)for r,n in ipairs(i)do
if r>1 and r<#i then
...
local r=mist.utils.get2DDist(e.polygonPoints[1],e.polygonPoints[#e.polygonPoints])if r<=25*25 then
...
function FDMMTerritory:buildFacilities()for r,e in pairs(self.facilities)do
e:buildFacility()end
end
...
while e>=0 do
local r=mist.vec.scalarMult(a,e*r)local r=mist.vec.add(i,r);COORDINATE:NewFromVec2(mist.utils.makeVec2(r)):Smoke(t)e=e-1
end
end
...
do
function fdmm.territory.createTerritories()fdmm.territories={[fdmm.enums.TerritoryType.Land]={},[fdmm.enums.TerritoryType.Sea]={},[fdmm.enums.TerritoryType.All]={}}local r={[fdmm.consts.TerritoryGNPrefix.Define]=fdmm.config.gpCache[fdmm.consts.TerritoryGNPrefix.Define]or{},[fdmm.consts.TerritoryGNPrefix.Linkage]=fdmm.config.gpCache[fdmm.consts.TerritoryGNPrefix.Linkage]or{}}for e,r in pairs(r[fdmm.consts.TerritoryGNPrefix.Define])do
local e=FDMMTerritory.newFromGroup(e,r)fdmm.territories[e.type][e.name]=e
fdmm.territories.all[e.name]=e
end
for e,r in pairs(r[fdmm.consts.TerritoryGNPrefix.Linkage])do
local i=fdmm.utils.removeGroupingPrefix(e)local r=fdmm.territories.all[i]if r~=nil then
for i,n in ipairs(mist.getGroupRoute(e,false))do
if string.isEmpty(n.name)then
local n=fdmm.territory.closestTerritoryToPoint(fdmm.utils.rposFromRPoint(n),r.type)if n~=nil then
...
function fdmm.territory.createFacilities()fdmm.facilities={[fdmm.enums.FacilityType.Airbase]={},[fdmm.enums.FacilityType.ArmsPlant]={},[fdmm.enums.FacilityType.CommandCenter]={},[fdmm.enums.FacilityType.FARP]={},[fdmm.enums.FacilityType.OilField]={},[fdmm.enums.FacilityType.Port]={},[fdmm.enums.FacilityType.UnitFactory]={},[fdmm.enums.FacilityType.All]={}}local r={[fdmm.consts.TerritoryGNPrefix.Facility]=fdmm.config.gpCache[fdmm.consts.TerritoryGNPrefix.Facility]or{}}local e=table.concatedWith(fdmm.consts.TerritoryGNPrefix,fdmm.consts.TerritoryWPFacilityPrefix)for a,r in pairs(r[fdmm.consts.TerritoryGNPrefix.Facility])do
local n=fdmm.utils.removeGroupingPrefix(a)local i=fdmm.territories.all[n]if i~=nil then
for o,r in ipairs(mist.getGroupRoute(a,false))do
if string.isNotEmpty(r.name)and(r.name..'_')~=fdmm.consts.TerritoryGNPrefix.Facility then
local t,e,s=fdmm.utils.getGroupingComponentsWithSNC(r.name,e,nil)if t==fdmm.consts.TerritoryWPFacilityPrefix.Airbase and string.isNotEmpty(e)then
local e=fdmm.facilities.all[e]or FDMMAirbase.new(e,fdmm.utils.rposFromRPoint(r),n)fdmm.facilities.airbase[e.name]=e
...
local e=fdmm.facilities.all[e]or FDMMArmsPlant.new(e,fdmm.utils.rposFromRPoint(r),n)fdmm.facilities.armsPlant[e.name]=e
fdmm.facilities.all[e.name]=e
i:addFacility(e)elseif t==fdmm.consts.TerritoryWPFacilityPrefix.CommandCenter and string.isNotEmpty(e)then
local e=fdmm.facilities.all[e]or FDMMCommandCenter.new(e,fdmm.utils.rposFromRPoint(r),n)fdmm.facilities.commandCenter[e.name]=e
fdmm.facilities.all[e.name]=e
i:addFacility(e)elseif t==fdmm.consts.TerritoryWPFacilityPrefix.FARP and string.isNotEmpty(e)then
local e=fdmm.facilities.all[e]or FDMMFARP.new(e,fdmm.utils.rposFromRPoint(r),n)fdmm.facilities.farp[e.name]=e
fdmm.facilities.all[e.name]=e
i:addFacility(e)elseif t==fdmm.consts.TerritoryWPFacilityPrefix.OilField and string.isNotEmpty(e)then
local e=fdmm.facilities.all[e]or FDMMOilField.new(e,fdmm.utils.rposFromRPoint(r),n)fdmm.facilities.oilField[e.name]=e
fdmm.facilities.all[e.name]=e
i:addFacility(e)elseif t==fdmm.consts.TerritoryWPFacilityPrefix.Port and string.isNotEmpty(e)then
local e=fdmm.facilities.all[e]or FDMMPort.new(e,fdmm.utils.rposFromRPoint(r),n)fdmm.facilities.port[e.name]=e
fdmm.facilities.all[e.name]=e
i:addFacility(e)elseif t==fdmm.consts.TerritoryWPFacilityPrefix.UnitFactory and string.isNotEmpty(e)then
local e=fdmm.facilities.all[e]or FDMMUnitFactory.new(e,fdmm.utils.rposFromRPoint(r),n)fdmm.facilities.unitFactory[e.name]=e
fdmm.facilities.all[e.name]=e
i:addFacility(e)elseif not(string.isEmpty(e)and string.isEmpty(s))then
env.error("Territory group '"..a.."' unknown facility WP '"..(r.name or'<nil>').."' at WP index "..o..'.')end
end
end
else
env.error("Territory facility group '"..a.."' failed to find territory named '"..n.."'.")end
end
end
function fdmm.territory.buildFacilities()for r,e in pairs(fdmm.territories.all)do
...
env.info('--FDMM Territories Dump--')env.info('  Sea Territories:')for r,e in pairs(fdmm.territories.sea)do
_envInfoTerritory(r,e)end
env.info('  Land Territories:')for r,e in pairs(fdmm.territories.land)do
_envInfoTerritory(r,e)end
end
end
...
setmetatable(FDMMFacility,{__call=function(e,...)return e.new(...)end})function FDMMFacility.new(i,n,r)local e=setmetatable({},FDMMFacility)e.name=i
e.centerPoint=n
e.territoryName=r
return e
end
...
setmetatable(FDMMAirbase,{__index=FDMMFacility,__call=function(e,...)return e.new(...)end})function FDMMAirbase.new(e,r,n)local e=setmetatable(FDMMFacility.new(e,r,n),FDMMAirbase)return e
end
...
setmetatable(FDMMArmsPlant,{__index=FDMMFacility,__call=function(e,...)return e.new(...)end})function FDMMArmsPlant.new(e,r,n)local e=setmetatable(FDMMFacility.new(e,r,n),FDMMArmsPlant)return e
end
...
setmetatable(FDMMFARP,{__index=FDMMFacility,__call=function(e,...)return e.new(...)end})function FDMMFARP.new(e,n,r)local e=setmetatable(FDMMFacility.new(e,n,r),FDMMFARP)return e
end
...
setmetatable(FDMMOilField,{__index=FDMMFacility,__call=function(e,...)return e.new(...)end})function FDMMOilField.new(n,e,r)local e=setmetatable(FDMMFacility.new(n,e,r),FDMMOilField)return e
end
...
setmetatable(FDMMUnitFactory,{__index=FDMMFacility,__call=function(e,...)return e.new(...)end})function FDMMUnitFactory.new(r,e,n)local e=setmetatable(FDMMFacility.new(r,e,n),FDMMUnitFactory)return e
end
