...
function fdmm.config.createGPCache()env.info("FDDM: Creating GP cache...")fdmm.config.gpCache={}local i={'AIRB_','ARMP_','CMDC_','FARP_','OILF_','PORT_','UNTF_'}for n,t in pairs(mist.DBs.groupsByName)do
...
function fdmm.utils.getGroupingComponents(e)assert(e,"Nonnull parameter: name")local t,n
e=fdmm.utils.removeNumericSuffix(e)e,t=fdmm.utils.removeGroupingPrefix(e)e,n=fdmm.utils.removeGroupingSuffix(e)return t,e,n
end
function fdmm.utils.getGroupingComponentsWithSNC(e,i,r)assert(e,"Nonnull parameter: name")local n,e,t=fdmm.utils.getGroupingComponents(e)if string.isEmpty(n)and table.contains(i or{},(e..'_'))then
n=(e..'_')e=nil
elseif string.isEmpty(t)and table.contains(r or{},('_'..e))then
t=('_'..e)e=nil
end
return n,e,t
end
...
function string.hasAnySuffix(e,n)assert(e,'Nonnull parameter: str')assert(n,'Nonnull parameter: suffixList')for t,n in pairs(n)do
...
function table.keysList(n)assert(n,'Nonnull parameter: tbl')local e={}for n,t in pairs(n)do
...
local function i(n,i)local function t(e,n)for e,t in pairs(e)do
if not fdmm.unitTypes._isReservedKey(e)then
if not n[e]then
n[e]=t
elseif n[e]~=t then
...
local function e(e)for n,t in pairs(e.All)do
if not fdmm.unitTypes._isReservedKey(n)then
local n=fdmm.unitTypes.getUnitType(t)if not e.ReportNaming[n]then
e.ReportNaming[n]=n
end
end
end
...
local n=e.Name
local e=unit_aliases._rev[n]local t=fdmm.consts.UnitType.All._rev[n]if not t and e then
if type(e)=='string'then
e={e}end
for i,e in ipairs(e)do
t=fdmm.consts.UnitType.All._rev[e]if t then
env.warning("  unitTypeAlias=['"..e.."'] is now unitType=['"..n.."'].")break
end
end
end
if not t then
env.info("  Missing fdmmUnitType for unitType=['"..(n or"<null>").."'] from "..i.."["..r.."].")end
end
end
...
function fdmm.unitTypes.whichAvailableToCountryIn(e,i,n)local t={}local a=string.lower(i)for i,r in pairs(e)do
...
function fdmm.unitTypes.anyListedUnder(n,e)fdmm.utils.ensureReversedDict(e)for n,t in pairs(n)do
if not string.hasPrefix(n,'_')then
local n=fdmm.unitTypes.getUnitType(t)if e._rev[n]then
return true
end
end
end
...
local n,a=1,table.getn(self.ranges)while n<a do
local i=math.floor((n+a)/2)local o=self.ranges[i]if e(o)>r then
a=i
elseif t(o)<r then
n=i+1
else
...
if n+1<=i and s(e(self.ranges[n+1])-1,self.ranges[n])then
self.ranges[n+1][1]=e(self.ranges[n])table.remove(self.ranges,n)end
if n-1>=1 and s(t(self.ranges[n-1])+1,self.ranges[n])then
...
function FDMMYearRange:addYearRange(o,a)assert(type(o)=='number',"Invalid parameter: startYear")assert(type(a)=='number',"Invalid parameter: endYear")local r={o,a}local n=table.getn(self.ranges)if n==0 or(o<=e(self.ranges[1])and a>=t(self.ranges[n]))then
self.ranges={r}self:_checkValidRangeAt(1)return
end
local n=self:_indexOfRangeAdjToOrCont(o)local m=n and self.ranges[n]or nil
if m then
if d(m,r)then
return
elseif a<e(m)then
...
local l={a,r}local i=self:_indexOfRangeAdjToOrCont(a)local o=i and self.ranges[i]or nil
local n=self:_indexOfRangeAdjToOrCont(r)local m=n and self.ranges[n]or nil
local d=s(a,o)local u=s(r,m)local s=a<=e(o)local c=r<e(m)local e=r>=t(m)if s and(c or e)then
if c then n=n-1 end
self:_collapseRangesFromTo(i,n)return
elseif i==n and d and u then
l={r+1,t(o)}self.ranges[i][2]=a-1
table.insert(self.ranges,i+1,l)self:_checkValidRangeAt(i)self:_checkValidRangeAt(i+1)return
else
local o,t=i,n
if not s then o=i+1 end
if not e then t=n-1 end
n=n-self:_collapseRangesFromTo(o,t)if not s and d then
self.ranges[i][2]=a-1
self:_checkValidRangeAt(i)elseif not e and u then
self.ranges[n][1]=r+1
self:_checkValidRangeAt(n)end
return
end
...
local t,n=pcall(e._yearRange.addYearsFromList,e._yearRange,e.Active)if not t then
...
local t,n=pcall(e._yearRange.removeYearsFromList,e._yearRange,e.Inactive)if not t then
...
function fdmm.regimentTypes._objForYear(e,i)if type(e)=='table'then
local t=nil
for r,n in ipairs(table.sortedKeysList(e))do
assert(type(n)=='number',"Invalid format: startYear")local e=e[n]assert(type(e)~='nil',"Invalid format.")if i==n then return e end
if i<n then return t or e end
t=e
end
return t
end
...
function fdmm.regimentTypes.getObjReportNameIn(t,n)local e=fdmm.regimentTypes._objForYear(t.ReportNaming,n)if string.isNotEmpty(e)then
local i=fdmm.regimentTypes.getObjNameIn(t,n)local n=fdmm.regimentTypes.getUnitHullNumberingIn(t,n)if string.isNotEmpty(i)and string.contains(e,'%N')then
e=e:gsub('%%N',i)end
if string.isNotEmpty(n)and string.contains(e,'%H')then
e=e:gsub('%%H',n)end
return e
end
return fdmm.regimentTypes._objForYear(t.Name,n)end
...
function fdmm.regimentTypes.dumpRegimentYearlyAvailability(o,r,a)env.info("FDMM: Dumping regiment yearly availability for years "..tostring(r).." to "..tostring(a).."...")local function i(t,e,n)env.info("FDMM: "..n.."Regiment: "..fdmm.regimentTypes.getObjReportNameIn(t,e))for i,t in pairs(t.Units or{})do
...
function FDMMRunLoop:startAsCallback(e)assert(e,"Nonnull parameter: timeInterval")assert(not self.isStarted,"Cannot start run loop more than once.")self.isStarted=true
self.timeInterval=e
timer.scheduleFunction(self.resume,self,timer.getTime()+self.timeInterval)end
function FDMMRunLoop:startAsCoroutine(e)assert(e,"Nonnull parameter: timeInterval")assert(not self.isStarted,"Cannot start run loop more than once.")self.isStarted=true
self.timeInterval=e
self.coroutine=coroutine.create(function()repeat
self:_main()until self.shouldExit
end)timer.scheduleFunction(self.resume,self,timer.getTime()+self.timeInterval)end
...
fdmm.runLoop.mainLoop:runIfActiveElseEnqueueAsync(function()timer.scheduleFunction(self.resume,self,timer.getTime()+self.timeInterval)end)self.isRunning=false
if self.coroutine then
coroutine.yield()end
else
...
function FDMMRunLoop:_main()self.isRunning=true
self.runStartTime=timer.getTime()local e,n=pcall(self.main,self)if not e then
...
function FDMMRunLoop:timeSpentActive()if self:isActiveLoop()then
...
n(e and table.unpack(e))else
local t=true
self:enqueueAsync(function()local n,e=pcall(n,table.unpack(e))t=false
return n,e
end,nil)while t do
...
setmetatable(FDMMResourceUnit,{__call=function(e,...)return e.new(...)end,})function FDMMResourceUnit.new(n,o,r,a,i,t)local e=setmetatable({},FDMMResourceUnit)e.fuelRP=n
e.armsRP=o
e.unitRP=r
e.maxFuelRP=a
e.maxArmsRP=i
...
function FDMMCargoRoute:addFacilityLocation(t,n,e)table.insert(self.facilityLocs,{facilityName=t,facilityPoint=n,ingressPoints=e or{}})end
function FDMMCargoRoute:addDirectionalLink(t,e,n)table.insert(self.dirLinks,{directionName=t,navigationPoint=e,ingressPoints=n or{}})end
...
local d={[fdmm.consts.CargoRouteGNPrefix.Land]=fdmm.config.gpCache[fdmm.consts.CargoRouteGNPrefix.Land]or{},[fdmm.consts.CargoRouteGNPrefix.Train]=fdmm.config.gpCache[fdmm.consts.CargoRouteGNPrefix.Train]or{},[fdmm.consts.CargoRouteGNPrefix.Air]=fdmm.config.gpCache[fdmm.consts.CargoRouteGNPrefix.Air]or{},[fdmm.consts.CargoRouteGNPrefix.Sea]=fdmm.config.gpCache[fdmm.consts.CargoRouteGNPrefix.Sea]or{}}function _processGroup(o,e,a)local n=fdmm.utils.removeNumericSuffix(fdmm.utils.removeGroupingPrefix(o))local t=fdmm.territories.all[n]if t~=nil then
local l=fdmm.cargoRoutes[n][a]local u=mist.getGroupRoute(o,false)local d=fdmm.utils.getFaction(string.notEmptyElse(e.units[1].unitName,e.country))local m=a:upperFirst()local c=table.concatedWith(fdmm.consts.CargoRouteGNPrefix,fdmm.consts.CargoRouteWPRoutePrefix[m])local e=nil
...
local d,i,t=fdmm.utils.getGroupingComponentsWithSNC(e.name,c,fdmm.consts.CargoRouteWPRouteSuffix)if d==fdmm.consts.CargoRouteGNPrefix[m]then
...
function fdmm.territory.closestTerritoryToPoint(e,t)local i=mist.utils.makeVec2(e)local e=nil
local n=0
local t=t or fdmm.enums.TerritoryType.All
local t=fdmm.territories[t]for r,t in pairs(t)do
local i=mist.utils.get2DDistSqrd(i,t.centerPoint)if e==nil or i<n then
e=t
n=i
end
end
...
fdmm.territory.buildFacilities()fdmm.runLoop.startRunLoops()end
