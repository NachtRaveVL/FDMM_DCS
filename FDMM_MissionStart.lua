...
local t=t:sub(1,i-1)if not fdmm.config.gpCache[e][t]then
fdmm.config.gpCache[e][t]={}end
fdmm.config.gpCache[e][t][n]=r
else
...
function fdmm.utils.getGroupingComponentsWithSNC(e,i,t)assert(e,'Nonnull parameter: name')local r,e,n=fdmm.utils.getGroupingComponents(e)if string.isEmpty(r)and table.contains(i or{},(e..'_'))then
r=(e..'_')e=nil
...
function string.contains(n,e)assert(n,'Nonnull parameter: str')assert(e,'Nonnull parameter: otherStr')return string.find(n,e,0,true)~=nil
end
...
function string.hasAnyPrefix(e,n)assert(e,'Nonnull parameter: str')assert(n,'Nonnull parameter: prefixList')for r,n in pairs(n)do
...
function string.hasAnySuffix(n,e)assert(n,'Nonnull parameter: str')assert(e,'Nonnull parameter: suffixList')for r,e in pairs(e)do
...
function table.contains(e,n)assert(e,'Nonnull parameter: tbl')assert(n,'Nonnull parameter: value')for r,e in pairs(e)do
if e==n then
return true
end
end
...
function table.concatedWith(n,r)assert(n or r,'Nonnull parameter: tbl1 or tbl2')local e={}if n then
...
function table.concatedWithi(e,r)assert(e or r,'Nonnull parameter: seq1 or seq2')local n={}if e then
for r,e in ipairs(e)do
table.insert(n,e)end
end
if r then
for r,e in ipairs(r)do
table.insert(n,e)end
end
return n
end
function table.concatWithi(n,e)assert(n,'Nonnull parameter: seq1')if e then
for r,e in ipairs(e)do
table.insert(n,e)end
end
end
function table.keysList(e)assert(e,'Nonnull parameter: tbl')local n={}for e,r in pairs(e)do
table.insert(n,e)end
return n
end
...
if not e._revShape[i]then
e._revShape[i]=n
elseif e._revShape[i]~=n then
env.error("Group all list shape reverse discrepancy for shapeName='"..i.."', groupAllList._revShape[shapeName]='"..e._revShape[i].."' ~= fdmmUnitType='"..n.."'.")end
end
elseif e[n]~=r then
...
o(s,s.All)end
local function t(e,r,t)for e,n in pairs(e)do
if not a(e)then
r[e]=(m[n]or t)..'::'..n
end
end
end
local function n(e,n)for e,r in pairs(e)do
if not a(e)then
local r=fdmm.unitTypes.getUnitType(r)if not n[e]then
n[e]=r
elseif n[e]~=r then
...
local function i(r,t)local function n(e,r)for e,n in pairs(e)do
if not a(e)then
if not r[e]then
r[e]=n
elseif r[e]~=n then
...
n(r.NATOReporting,t.NATOReporting)n(r.WTOReporting,t.WTOReporting)n(r.ReportNaming,t.ReportNaming)n(r.ProperNaming,t.ProperNaming)n(r.Nicknaming,t.Nicknaming)end
local function e(n)for e,r in pairs(n.All)do
if not a(e)then
local e=fdmm.unitTypes.getUnitType(r)if not n.ReportNaming[e]then
n.ReportNaming[e]=e
end
end
end
end
local function s()assert(db,"Missing module: db")assert(dbYears,"Missing module: dbYears")fdmm.consts.UnitType.Available={}for n,e in pairs(fdmm.consts.UnitType.All)do
if not a(n)then
local n={}if dbYears[e]then
local r=db.getHistoricalCountres(e)for t,r in pairs(r)do
local t,e=db.getYearsLocal(e,r)n[string.trim(r)]={t,e}end
else
n['ALL']={1900,9999}end
...
local function a()assert(JSON,"Missing module: JSON")local e=assert(io.open(l,'w'))local n=JSON:encode(fdmm.consts.UnitType.Available)e:write(n)e:close()end
local function m()assert(JSON,"Missing module: JSON")local e=assert(io.open(l,'r'))local n=e:read("*all")e:close()fdmm.consts.UnitType.Available=JSON:decode(n)end
o(fdmm.consts.UnitType.Plane)o(fdmm.consts.UnitType.Helicopter)o(fdmm.consts.UnitType.Ground)o(fdmm.consts.UnitType.Ship)o(fdmm.consts.UnitType.Train)t(fdmm.consts.UnitType.Plane.All,fdmm.consts.UnitType.Static.Plane,'Planes')t(fdmm.consts.UnitType.Helicopter.All,fdmm.consts.UnitType.Static.Helicopter,'Helicopters')t(fdmm.consts.UnitType.Ground.All,fdmm.consts.UnitType.Static.Ground,'Unarmed')t(fdmm.consts.UnitType.Train.All,fdmm.consts.UnitType.Static.Train,'Unarmed')t(fdmm.consts.UnitType.Ship.All,fdmm.consts.UnitType.Static.Ship,'Ships')o(fdmm.consts.UnitType.Static)n(fdmm.consts.UnitType.Plane.All,fdmm.consts.UnitType.All)n(fdmm.consts.UnitType.Helicopter.All,fdmm.consts.UnitType.All)n(fdmm.consts.UnitType.Ground.All,fdmm.consts.UnitType.All)n(fdmm.consts.UnitType.Train.All,fdmm.consts.UnitType.All)n(fdmm.consts.UnitType.Ship.All,fdmm.consts.UnitType.All)n(fdmm.consts.UnitType.Static.All,fdmm.consts.UnitType.All)table.concatWith(fdmm.consts.UnitType.Airborne,fdmm.consts.UnitType.Ground.Airborne)table.concatWith(fdmm.consts.UnitType.Amphibious,fdmm.consts.UnitType.Ground.Amphibious)table.concatWith(fdmm.consts.UnitType.CarrierBorne,fdmm.consts.UnitType.Plane.CarrierBorne)table.concatWith(fdmm.consts.UnitType.CarrierBorne,fdmm.consts.UnitType.Helicopter.CarrierBorne)table.concatWith(fdmm.consts.UnitType.HeavyWheeled,fdmm.consts.UnitType.Ground.HeavyWheeled)table.concatWith(fdmm.consts.UnitType.Marines,fdmm.consts.UnitType.Ground.Marines)table.concatWith(fdmm.consts.UnitType.PlayerControllable,fdmm.consts.UnitType.Plane.PlayerControllable)table.concatWith(fdmm.consts.UnitType.PlayerControllable,fdmm.consts.UnitType.Helicopter.PlayerControllable)table.concatWith(fdmm.consts.UnitType.Unavailable,fdmm.consts.UnitType.Plane.Unavailable)table.concatWith(fdmm.consts.UnitType.Unavailable,fdmm.consts.UnitType.Helicopter.Unavailable)table.concatWith(fdmm.consts.UnitType.Unavailable,fdmm.consts.UnitType.Ground.Unavailable)fdmm.utils.ensureReversedDict(fdmm.consts.UnitType.Plane.CarrierBorne)fdmm.utils.ensureReversedDict(fdmm.consts.UnitType.Plane.PlayerControllable)fdmm.utils.ensureReversedDict(fdmm.consts.UnitType.Plane.Unavailable)fdmm.utils.ensureReversedDict(fdmm.consts.UnitType.Helicopter.CarrierBorne)fdmm.utils.ensureReversedDict(fdmm.consts.UnitType.Helicopter.PlayerControllable)fdmm.utils.ensureReversedDict(fdmm.consts.UnitType.Helicopter.Unavailable)fdmm.utils.ensureReversedDict(fdmm.consts.UnitType.Ground.Airborne)fdmm.utils.ensureReversedDict(fdmm.consts.UnitType.Ground.Amphibious)fdmm.utils.ensureReversedDict(fdmm.consts.UnitType.Ground.HeavyWheeled)fdmm.utils.ensureReversedDict(fdmm.consts.UnitType.Ground.Marines)fdmm.utils.ensureReversedDict(fdmm.consts.UnitType.Ground.Unavailable)fdmm.utils.ensureReversedDict(fdmm.consts.UnitType.All)fdmm.utils.ensureReversedDict(fdmm.consts.UnitType.Airborne)fdmm.utils.ensureReversedDict(fdmm.consts.UnitType.Amphibious)fdmm.utils.ensureReversedDict(fdmm.consts.UnitType.CarrierBorne)fdmm.utils.ensureReversedDict(fdmm.consts.UnitType.HeavyWheeled)fdmm.utils.ensureReversedDict(fdmm.consts.UnitType.Marines)fdmm.utils.ensureReversedDict(fdmm.consts.UnitType.PlayerControllable)fdmm.utils.ensureReversedDict(fdmm.consts.UnitType.Unavailable)r(fdmm.consts.UnitType.Plane)r(fdmm.consts.UnitType.Helicopter)r(fdmm.consts.UnitType.Ground)r(fdmm.consts.UnitType.Ship)r(fdmm.consts.UnitType.Train)r(fdmm.consts.UnitType.Static)e(fdmm.consts.UnitType.Plane)e(fdmm.consts.UnitType.Helicopter)e(fdmm.consts.UnitType.Ground)e(fdmm.consts.UnitType.Ship)e(fdmm.consts.UnitType.Train)i(fdmm.consts.UnitType.Plane,fdmm.consts.UnitType.Static)i(fdmm.consts.UnitType.Helicopter,fdmm.consts.UnitType.Static)i(fdmm.consts.UnitType.Ground,fdmm.consts.UnitType.Static)i(fdmm.consts.UnitType.Ship,fdmm.consts.UnitType.Static)i(fdmm.consts.UnitType.Train,fdmm.consts.UnitType.Static)e(fdmm.consts.UnitType.Static)table.concatWith(fdmm.consts.UnitType.Nicknaming,fdmm.consts.UnitType.Plane.Nicknaming)table.concatWith(fdmm.consts.UnitType.Nicknaming,fdmm.consts.UnitType.Helicopter.Nicknaming)table.concatWith(fdmm.consts.UnitType.Nicknaming,fdmm.consts.UnitType.Ground.Nicknaming)table.concatWith(fdmm.consts.UnitType.Nicknaming,fdmm.consts.UnitType.Train.Nicknaming)table.concatWith(fdmm.consts.UnitType.Nicknaming,fdmm.consts.UnitType.Ship.Nicknaming)table.concatWith(fdmm.consts.UnitType.Nicknaming,fdmm.consts.UnitType.Static.Nicknaming)if db and dbYears and fdmm.utils.isDebugFlagSet()then
s()a()else
m()end
end
function fdmm.unitTypes.crossRefEntries()if db then
env.info("FDMM: Cross-referencing units...")local function e(i,e)for t,e in ipairs(e)do
local n=e.Name
...
if not r then
env.info("  Missing fdmmUnitType for unitType=['"..(n or"<null>").."'] from "..i.."["..t.."].")end
end
end
...
local n={}for r,t in pairs(dbYears)do
table.insert(n,{Name=r})end
e('dbYears',n)end
end
end
function fdmm.unitTypes.whichAvailableToCountryIn(e,a,i)local t={}for r,n in pairs(e)do
if not string.hasPrefix(r,'_')then
local e=fdmm.unitTypes.getUnitType(n)if not fdmm.unitTypes.isListedUnder(e,fdmm.consts.UnitType.Unavailable)then
local e=fdmm.consts.UnitType.Available[e]local e=e[a]or e['ALL']if e and i>=e[1]and i<=e[2]then
t[r]=n
end
end
end
...
function fdmm.unitTypes.anyAvailableToCountryIn(e,r,n)for e,t in pairs(e)do
...
local e=fdmm.consts.UnitType.Available[e]local e=e[r]or e['ALL']if e and n>=e[1]and n<=e[2]then
return true
end
end
end
...
function fdmm.unitTypes.isAvailableToCountryIn(e,r,n)if not fdmm.unitTypes.isListedUnder(e,fdmm.consts.UnitType.Unavailable)then
local e=fdmm.consts.UnitType.Available[e]local e=e[r]or e['ALL']if e and n>=e[1]and n<=e[2]then
return true
end
end
...
function fdmm.unitTypes.whichListedUnder(e,r)fdmm.utils.ensureReversedDict(r)local t={}for n,e in pairs(e)do
if not string.hasPrefix(n,'_')then
local i=fdmm.unitTypes.getUnitType(e)if r._rev[i]then
t[n]=e
end
end
end
...
function fdmm.unitTypes.getUnitType(n)local r,r,e=fdmm.utils.splitTuple(n)e=e or n
return e
end
...
local i=r or fdmm.unitTypes.getUnitTypeGroup(n)local o=i[a][n]local r,t,s=fdmm.utils.splitTuple(o)r=r or o
...
function fdmm.unitTypes.dumpUnitReportNames()env.info("FDMM: Dumping unit report names...")local function t(e)for r,n in pairs(table.sortedKeysList(e.All))do
if not string.hasPrefix(n,'_')then
local r=e.All[n]local r=fdmm.unitTypes.getUnitType(r)env.info("  ['"..n.."'] = '"..r.."':")env.info("    NATO: "..fdmm.unitTypes.getNATOUnitReportName(r,e))env.info("    WTO : "..fdmm.unitTypes.getWTOUnitReportName(r,e))end
end
end
...
local function r(e)if type(e)~='table'then return nil end
...
local t=math.floor((e+a)/2)local i=self.ranges[t]if n(i)>o then
a=t
elseif r(i)<o then
e=t+1
else
...
function FDMMYearRange:_collapseRangesFromTo(t,e)local n=0
local r=table.getn(self.ranges)while e>=t do
if e>=1 and e<=r then
table.remove(self.ranges,e)r=r-1
n=n+1
end
e=e-1
end
...
function FDMMYearRange:_checkCombineNeighborsAt(e)local t=table.getn(self.ranges)if e>=1 and e<=t then
if e+1<=t and s(n(self.ranges[e+1])-1,self.ranges[e])then
self.ranges[e+1][1]=n(self.ranges[e])table.remove(self.ranges,e)end
if e-1>=1 and s(r(self.ranges[e-1])+1,self.ranges[e])then
self.ranges[e-1][2]=r(self.ranges[e])table.remove(self.ranges,e)end
self:_checkValidRangeAt(e)end
end
function FDMMYearRange:_checkValidRangeAt(e)local t=table.getn(self.ranges)if e>=1 and e<=t then
local t=self.ranges[e]if n(t)>r(t)then
...
return r(self.ranges[table.getn(self.ranges)])end
function FDMMYearRange:getYearList()local t={}for i,e in ipairs(self.ranges or{})do
if n(e)~=r(e)then
table.insert(t,e)else
...
function FDMMYearRange:addYearRange(o,a)assert(type(o)=='number',"Invalid parameter: startYear")assert(type(a)=='number',"Invalid parameter: endYear")local i={o,a}local e=table.getn(self.ranges)if e==0 or(o<=n(self.ranges[1])and a>=r(self.ranges[e]))then
self.ranges={i}self:_checkValidRangeAt(1)return
end
...
return
elseif a<n(m)then
table.insert(self.ranges,e,i)self:_checkCombineNeighborsAt(e)return
end
end
local t=self:_indexOfRangeAdjToOrCont(a)local l=t and self.ranges[t]or nil
if l and o>r(l)then
table.insert(self.ranges,i)self:_checkCombineNeighborsAt(t+1)return
end
local d=s(o,m)local o=s(a,l)if d and o then
i={n(m),r(l)}self:_collapseRangesFromTo(e,t)table.insert(self.ranges,e,i)self:_checkCombineNeighborsAt(e)return
elseif not d and not o then
local n=a<n(l)if n then t=t-1 end
self:_collapseRangesFromTo(e,t)table.insert(self.ranges,e,i)self:_checkCombineNeighborsAt(e)return
else
if d then
self:_collapseRangesFromTo(e+1,t)self.ranges[e][2]=r(i)self:_checkCombineNeighborsAt(e)elseif o then
t=t-self:_collapseRangesFromTo(e,t-1)self.ranges[t][1]=n(i)self:_checkCombineNeighborsAt(t)end
return
end
end
...
function FDMMYearRange:removeYearRange(a,i)assert(type(a)=='number',"Invalid parameter: startYear")assert(type(i)=='number',"Invalid parameter: endYear")local e=table.getn(self.ranges)if e==0 or(a<=n(self.ranges[1])and i>=r(self.ranges[e]))then
self.ranges={}return
end
local m={a,i}local e=self:_indexOfRangeAdjToOrCont(a)local o=e and self.ranges[e]or nil
local t=self:_indexOfRangeAdjToOrCont(i)local l=t and self.ranges[t]or nil
local d=s(a,o)local u=s(i,l)local s=a<=n(o)local c=i<n(l)local n=i>=r(l)if s and(c or n)then
if c then t=t-1 end
self:_collapseRangesFromTo(e,t)return
elseif e==t and d and u then
m={i+1,r(o)}self.ranges[e][2]=a-1
table.insert(self.ranges,e+1,m)self:_checkValidRangeAt(e)self:_checkValidRangeAt(e+1)return
else
local r,o=e,t
if not s then r=e+1 end
if not n then o=t-1 end
t=t-self:_collapseRangesFromTo(r,o)if not s and d then
self.ranges[e][2]=a-1
self:_checkValidRangeAt(e)elseif not n and u then
self.ranges[t][1]=i+1
self:_checkValidRangeAt(t)end
return
end
end
...
local r,n=pcall(e._yearRange.removeYearsFromList,e._yearRange,e.Inactive)if not r then
...
local r,e=e._yearRange:startYear(),e._yearRange:endYear()if type(r)=='number'and r<n[1]then n[1]=r end
...
t(fdmm.consts.RegimentType.Caucasus.Ship.USA.Divisions.SixthFleet,{9999,0})end
function fdmm.regimentTypes._objForYear(n,i)if type(n)=='table'then
local t=nil
for e,r in ipairs(table.sortedKeysList(n))do
assert(type(r)=='number',"Invalid format: startYear")local e=n[r]assert(type(e)~='nil',"Invalid format.")if i==r then return e end
if i<r then return t or e end
t=e
end
return t
end
...
function fdmm.regimentTypes.getObjReportNameIn(r,n)local e=fdmm.regimentTypes._objForYear(r.ReportNaming,n)if string.isNotEmpty(e)then
local t=fdmm.regimentTypes.getObjNameIn(r,n)local n=fdmm.regimentTypes.getUnitHullNumberingIn(r,n)if string.isNotEmpty(t)and string.contains(e,'%N')then
e=e:gsub('%%N',t)end
if string.isNotEmpty(n)and string.contains(e,'%H')then
e=e:gsub('%%H',n)end
return e
end
return fdmm.regimentTypes._objForYear(r.Name,n)end
...
function fdmm.regimentTypes.dumpRegimentYearlyAvailability(o,a,i)env.info("FDMM: Dumping regiment yearly availability for years "..tostring(a).." to "..tostring(i).."...")local function t(n,e,r)env.info("FDMM: "..r.."Regiment: "..fdmm.regimentTypes.getObjReportNameIn(n,e))for t,n in pairs(n.Units or{})do
if fdmm.regimentTypes.isRegimentUnitAvailableIn(n,e)then
env.info("FDMM: "..r.."  Unit: "..fdmm.regimentTypes.getObjReportNameIn(n,e))end
end
for i,n in pairs(n.Regiments or{})do
t(n,e,r.."  ")end
if n.Reinforcements then
env.info("FDMM: "..r.."  Reinforcements:")for a,i in pairs(n.Reinforcements.Units or{})do
if fdmm.regimentTypes.isRegimentUnitAvailableIn(i,e)then
env.info("FDMM: "..r.."    Unit: "..fdmm.regimentTypes.getObjReportNameIn(i,e))end
...
for e=a,i do
env.info("FDMM: Year: "..e)t(o,e,"")end
end
function fdmm.regimentTypes.dumpRegimentActiveLists(i)env.info("FDMM: Dumping regiment year list...")local r=fdmm.setup.equipmentYear
local function t(n,e)env.info("FDMM: "..e.."Regiment: "..fdmm.regimentTypes.getObjReportNameIn(n,r))env.info("FDMM: "..e.."  _Active: "..JSON:encode(n._Active))for t,n in pairs(n.Units or{})do
env.info("FDMM: "..e.."  Unit: "..fdmm.regimentTypes.getObjReportNameIn(n,r))env.info("FDMM: "..e.."    _Active: "..JSON:encode(n._yearRange:getYearList()))end
...
setmetatable(FDMMResourceUnit,{__call=function(e,...)return e.new(...)end,})function FDMMResourceUnit.new(o,i,a,t,n,r)local e=setmetatable({},FDMMResourceUnit)e.fuelRP=o
e.armsRP=i
e.unitRP=a
e.maxFuelRP=t
e.maxArmsRP=n
...
function FDMMCargoRoute:addSpawnLocation(r,e,n)table.insert(self.spawnLocs,{spawnFaction=r,spawnPoint=e,egressPoints=n or{}})end
function FDMMCargoRoute:addFacilityLocation(n,e,r)table.insert(self.facilityLocs,{facilityName=n,facilityPoint=e,ingressPoints=r or{}})end
function FDMMCargoRoute:addDirectionalLink(n,r,e)table.insert(self.dirLinks,{directionName=n,navigationPoint=r,ingressPoints=e or{}})end
...
local u={[fdmm.consts.CargoRouteGNPrefix.Land]=fdmm.config.gpCache[fdmm.consts.CargoRouteGNPrefix.Land]or{},[fdmm.consts.CargoRouteGNPrefix.Train]=fdmm.config.gpCache[fdmm.consts.CargoRouteGNPrefix.Train]or{},[fdmm.consts.CargoRouteGNPrefix.Air]=fdmm.config.gpCache[fdmm.consts.CargoRouteGNPrefix.Air]or{},[fdmm.consts.CargoRouteGNPrefix.Sea]=fdmm.config.gpCache[fdmm.consts.CargoRouteGNPrefix.Sea]or{}}function _processGroup(a,n,o)local e=fdmm.utils.removeNumericSuffix(fdmm.utils.removeGroupingPrefix(a))local r=fdmm.territories.all[e]if r~=nil then
local m=fdmm.cargoRoutes[e][o]local u=mist.getGroupRoute(a,false)local d=fdmm.utils.getFaction(string.notEmptyElse(n.units[1].unitName,n.country))local l=o:upperFirst()local c=table.concatedWith(fdmm.consts.CargoRouteGNPrefix,fdmm.consts.CargoRouteWPRoutePrefix[l])local n=nil
local e={Spawn='spawn',Linkage='linkage',Facility='facility',TBD='_'}local r,i,s=nil,nil,{}function _updateScanner(t,l)if n==e.TBD and t~=nil and t~=e.TBD then
n=t
...
if n then
if n==e.Spawn then
if string.isNotEmpty(d)and i and s then
m:addSpawnLocation(d,i,s)else
env.error('Cargo '..o.." routing group '"..a.."' failed to parse spawn route '"..(r or'<nil>').."'.")end
elseif n==e.Linkage then
if string.isNotEmpty(r)and i and s then
m:addDirectionalLink(r,i,s)else
env.error('Cargo '..o.." routing group '"..a.."' failed to parse linkage route '"..(r or'<nil>').."'.")end
elseif n==e.Facility then
if string.isNotEmpty(r)and i and s then
m:addFacilityLocation(r,i,s)else
env.error('Cargo '..o.." routing group '"..a.."' failed to parse facility route '"..(r or'<nil>').."'.")end
elseif n~=e.TBD then
env.error('Cargo '..o.." routing group '"..a.."' encountered unrecognized scan mode '"..(t or'<nil>').."'.")end
r,i,s=nil,nil,{}end
n=t
r=l
end
...
local d,t,r=fdmm.utils.getGroupingComponentsWithSNC(n.name,c,fdmm.consts.CargoRouteWPRouteSuffix)if d==fdmm.consts.CargoRouteGNPrefix[l]then
if r==fdmm.consts.CargoRouteWPRouteSuffix.Egress or
r==fdmm.consts.CargoRouteWPRouteSuffix.Ingress then
_updateScanner(e.TBD,t)table.insert(s,fdmm.utils.rposFromRPoint(n))elseif r==fdmm.consts.CargoRouteWPRouteSuffix.Spawn then
_updateScanner(e.Spawn,t)i=fdmm.utils.rposFromRPoint(n)elseif r==fdmm.consts.CargoRouteWPRouteSuffix.Linkage then
_updateScanner(e.Linkage,t)i=fdmm.utils.rposFromRPoint(n)elseif r==fdmm.consts.CargoRouteWPRouteSuffix.Facility then
_updateScanner(e.Facility,t)i=fdmm.utils.rposFromRPoint(n)elseif not(string.isEmpty(t)and string.isEmpty(r))then
env.error('Cargo '..o.." routing group '"..a.."' unknown WP '"..(n.name or'<nil>').."' at WP index "..m..'.')end
elseif d==fdmm.consts.CargoRouteWPRoutePrefix[l].Spawn then
if string.isEmpty(r)then
_updateScanner(e.Spawn,t)i=fdmm.utils.rposFromRPoint(n)elseif r==fdmm.consts.CargoRouteWPRouteSuffix.Egress then
_updateScanner(e.Spawn,t)table.insert(s,fdmm.utils.rposFromRPoint(n))elseif not(string.isEmpty(t)and string.isEmpty(r))then
env.error('Cargo '..o.." routing group '"..a.."' unknown spawn WP '"..(n.name or'<nil>').."' at WP index "..m..'.')end
elseif d==fdmm.consts.CargoRouteWPRoutePrefix[l].Linkage then
if string.isEmpty(r)then
_updateScanner(e.Linkage,t)i=fdmm.utils.rposFromRPoint(n)elseif r==fdmm.consts.CargoRouteWPRouteSuffix.Ingress then
_updateScanner(e.Linkage,t)table.insert(s,fdmm.utils.rposFromRPoint(n))elseif not(string.isEmpty(t)and string.isEmpty(r))then
env.error('Cargo '..o.." routing group '"..a.."' unknown linkage WP '"..(n.name or'<nil>').."' at WP index "..m..'.')end
elseif d==fdmm.consts.CargoRouteWPRoutePrefix[l].Facility then
if string.isEmpty(r)then
_updateScanner(e.Facility,t)i=fdmm.utils.rposFromRPoint(n)elseif r==fdmm.consts.CargoRouteWPRouteSuffix.Ingress then
_updateScanner(e.Facility,t)table.insert(s,fdmm.utils.rposFromRPoint(n))elseif not(string.isEmpty(t)and string.isEmpty(r))then
env.error('Cargo '..o.." routing group '"..a.."' unknown facility WP '"..(n.name or'<nil>').."' at WP index "..m..'.')end
else
env.error('Cargo '..o.." routing group '"..a.."' unknown routing WP '"..(n.name or'<nil>').."' at WP index "..m..'.')end
end
end
_updateScanner(nil,nil)else
env.error('Cargo '..o.." routing group '"..a.."' failed to find territory with same name.")end
end
for n,e in pairs(u[fdmm.consts.CargoRouteGNPrefix.Land])do
_processGroup(n,e,fdmm.enums.CargoRouteType.Land)end
for e,n in pairs(u[fdmm.consts.CargoRouteGNPrefix.Train])do
...
e.polygonPoints={}local t=mist.getGroupPoints(n)for r,n in ipairs(t)do
if r>1 and r<#t then
...
for t,r in ipairs(mist.getGroupRoute(e,false))do
...
function fdmm.territory.createFacilities()fdmm.facilities={[fdmm.enums.FacilityType.Airbase]={},[fdmm.enums.FacilityType.ArmsPlant]={},[fdmm.enums.FacilityType.CommandCenter]={},[fdmm.enums.FacilityType.FARP]={},[fdmm.enums.FacilityType.OilField]={},[fdmm.enums.FacilityType.Port]={},[fdmm.enums.FacilityType.UnitFactory]={},[fdmm.enums.FacilityType.All]={}}local n={[fdmm.consts.TerritoryGNPrefix.Facility]=fdmm.config.gpCache[fdmm.consts.TerritoryGNPrefix.Facility]or{}}local e=table.concatedWith(fdmm.consts.TerritoryGNPrefix,fdmm.consts.TerritoryWPFacilityPrefix)for a,n in pairs(n[fdmm.consts.TerritoryGNPrefix.Facility])do
local r=fdmm.utils.removeGroupingPrefix(a)local t=fdmm.territories.all[r]if t~=nil then
...
local i,e,o=fdmm.utils.getGroupingComponentsWithSNC(n.name,e,nil)if i==fdmm.consts.TerritoryWPFacilityPrefix.Airbase and string.isNotEmpty(e)then
...
t:addFacility(e)elseif i==fdmm.consts.TerritoryWPFacilityPrefix.ArmsPlant and string.isNotEmpty(e)then
local e=fdmm.facilities.all[e]or FDMMArmsPlant.new(e,fdmm.utils.rposFromRPoint(n),r)fdmm.facilities.armsPlant[e.name]=e
fdmm.facilities.all[e.name]=e
t:addFacility(e)elseif i==fdmm.consts.TerritoryWPFacilityPrefix.CommandCenter and string.isNotEmpty(e)then
...
fdmm.facilities.all[e.name]=e
t:addFacility(e)elseif i==fdmm.consts.TerritoryWPFacilityPrefix.FARP and string.isNotEmpty(e)then
local e=fdmm.facilities.all[e]or FDMMFARP.new(e,fdmm.utils.rposFromRPoint(n),r)fdmm.facilities.farp[e.name]=e
fdmm.facilities.all[e.name]=e
t:addFacility(e)elseif i==fdmm.consts.TerritoryWPFacilityPrefix.OilField and string.isNotEmpty(e)then
local e=fdmm.facilities.all[e]or FDMMOilField.new(e,fdmm.utils.rposFromRPoint(n),r)fdmm.facilities.oilField[e.name]=e
fdmm.facilities.all[e.name]=e
t:addFacility(e)elseif i==fdmm.consts.TerritoryWPFacilityPrefix.Port and string.isNotEmpty(e)then
local e=fdmm.facilities.all[e]or FDMMPort.new(e,fdmm.utils.rposFromRPoint(n),r)fdmm.facilities.port[e.name]=e
fdmm.facilities.all[e.name]=e
t:addFacility(e)elseif i==fdmm.consts.TerritoryWPFacilityPrefix.UnitFactory and string.isNotEmpty(e)then
...
fdmm.facilities.all[e.name]=e
t:addFacility(e)elseif not(string.isEmpty(e)and string.isEmpty(o))then
env.error("Territory group '"..a.."' unknown facility WP '"..tostring(n.name).."' at WP index "..s..".")end
end
end
else
...
local n=fdmm.territories[n]for i,n in pairs(n)do
local r=mist.utils.get2DDistSqrd(r,n.centerPoint)if e==nil or r<t then
e=n
t=r
end
end
...
setmetatable(FDMMFacility,{__call=function(e,...)return e.new(...)end})function FDMMFacility.new(r,n,t)local e=setmetatable({},FDMMFacility)e.name=r
e.centerPoint=n
e.territoryName=t
return e
end
...
setmetatable(FDMMArmsPlant,{__index=FDMMFacility,__call=function(e,...)return e.new(...)end})function FDMMArmsPlant.new(r,e,n)local e=setmetatable(FDMMFacility.new(r,e,n),FDMMArmsPlant)return e
end
...
setmetatable(FDMMCommandCenter,{__index=FDMMFacility,__call=function(e,...)return e.new(...)end})function FDMMCommandCenter.new(n,e,r)local e=setmetatable(FDMMFacility.new(n,e,r),FDMMCommandCenter)return e
end
...
setmetatable(FDMMFARP,{__index=FDMMFacility,__call=function(e,...)return e.new(...)end})function FDMMFARP.new(n,e,r)local e=setmetatable(FDMMFacility.new(n,e,r),FDMMFARP)return e
end
...
setmetatable(FDMMPort,{__index=FDMMFacility,__call=function(e,...)return e.new(...)end})function FDMMPort.new(r,n,e)local e=setmetatable(FDMMFacility.new(r,n,e),FDMMPort)return e
end
...
setmetatable(FDMMUnitFactory,{__index=FDMMFacility,__call=function(e,...)return e.new(...)end})function FDMMUnitFactory.new(e,r,n)local e=setmetatable(FDMMFacility.new(e,r,n),FDMMUnitFactory)return e
end
