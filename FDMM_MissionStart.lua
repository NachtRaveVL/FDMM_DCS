...
do
fdmm.config.Config={_tableVer=1,updatedDCSDetected=false,lastDCSVersionSeen=nil,updatedFDMMDetected=false,lastFDMMVersionSeen=nil}function fdmm.config.runUserSetupScript()env.info("FDDM: Running user setup script...")fdmm.setup={}local e,n=pcall(dofile,fdmm.fullPath..'FDMM_Setup.lua')if e then
...
function fdmm.config.detectVersions()env.info("FDDM: Detecting versions...")local e=fdmm.utils.getDCSVersion()fdmm.config.Config.updatedDCSDetected=fdmm.config.Config.lastDCSVersionSeen==nil or
fdmm.config.Config.lastDCSVersionSeen~=e
fdmm.config.Config.lastDCSVersionSeen=e
if fdmm.config.Config.updatedDCSDetected then
env.info("FDMM:   ...New DCS version detected: v"..e)end
local e=fdmm.utils.getFDMMVersion()fdmm.config.Config.updatedFDMMDetected=fdmm.config.Config.lastFDMMVersionSeen==nil or
fdmm.config.Config.lastFDMMVersionSeen~=e
fdmm.config.Config.lastFDMMVersionSeen=e
if fdmm.config.Config.updatedFDMMDetected then
env.info("FDMM:   ...New FDMM version detected: v"..e)end
local i=fdmm.config.Config.updatedDCSDetected or
...
env.info("FDDM: Loading DCS JSON...")local n,e=pcall(n,'./Scripts/JSON')if n then JSON=e end
end
end
function fdmm.config.saveConfig()env.info("FDDM: Saving Config...")local e=fdmm.fullPath.."data/Config.json"fdmm.utils.encodeToJSONFile(fdmm.config.Config,e)end
function fdmm.config.loadConfigOrDefaults()env.info("FDDM: Loading Config (or defaults)...")local e=fdmm.fullPath.."data/Config.json"local n,e=pcall(fdmm.utils.decodeFromJSONFile,e)if n and e then
local n=fdmm.config.Config._tableVer
if n==e._tableVer then
fdmm.config.Config=e
else
env.info("FDDM:   ...Updating config to v"..n..".")table.concatWith(fdmm.config.Config,e)fdmm.config.Config._tableVer=n
end
else
env.info("FDDM:   ...Config not found, using defaults.")end
end
function fdmm.config.createGPCache()env.info("FDDM: Creating GP cache...")fdmm.config.gpCache={}local r={'AIRB_','ARMP_','CMDC_','FARP_','OILF_','PORT_','UNTF_'}for n,t in pairs(mist.DBs.groupsByName)do
...
local r=string.find(i,'_')if r~=nil then
local i=i:sub(1,r-1)if not fdmm.config.gpCache[e][i]then
...
function fdmm.utils.getFaction(e)assert(e,"Nonnull parameter: name")e=fdmm.utils.removeNumericSuffix(e)for i,n in pairs(fdmm.enums.Faction)do
...
function table.contains(n,e)assert(n,'Nonnull parameter: tbl')assert(e,'Nonnull parameter: value')for i,n in pairs(n)do
if n==e then
return true
end
end
...
function table.concatedWith(e,n)assert(e or n,'Nonnull parameter: tbl1 or tbl2')local i={}if e then
...
function table.concatedWithi(i,e)assert(i or e,'Nonnull parameter: seq1 or seq2')local n={}if i then
...
function table.keysList(n)assert(n,'Nonnull parameter: tbl')local e={}for n,i in pairs(n)do
table.insert(e,n)end
return e
end
...
o(a,a.All)end
local function a(e,t,i)for n,e in pairs(e)do
if not fdmm.unitTypes._isReservedKey(n)then
t[n]=(s[e]or i)..'::'..e
end
end
end
local function n(e,n)for e,i in pairs(e)do
...
local function i(e)if not e.NATOReporting then e.NATOReporting={}end
...
local function t(t,i)local function n(e,i)for e,n in pairs(e)do
if not fdmm.unitTypes._isReservedKey(e)then
if not i[e]then
i[e]=n
elseif i[e]~=n then
...
n(t.NATOReporting,i.NATOReporting)n(t.WTOReporting,i.WTOReporting)n(t.ReportNaming,i.ReportNaming)n(t.ProperNaming,i.ProperNaming)n(t.Nicknaming,i.Nicknaming)end
local function e(n)for e,i in pairs(n.All)do
if not fdmm.unitTypes._isReservedKey(e)then
local e=fdmm.unitTypes.getUnitType(i)if not n.ReportNaming[e]then
n.ReportNaming[e]=e
end
end
end
end
r(fdmm.consts.UnitType.Plane)r(fdmm.consts.UnitType.Helicopter)r(fdmm.consts.UnitType.Ground)r(fdmm.consts.UnitType.Ship)r(fdmm.consts.UnitType.Train)a(fdmm.consts.UnitType.Plane.All,fdmm.consts.UnitType.Static.Plane,'Planes')a(fdmm.consts.UnitType.Helicopter.All,fdmm.consts.UnitType.Static.Helicopter,'Helicopters')a(fdmm.consts.UnitType.Ground.All,fdmm.consts.UnitType.Static.Ground,'Unarmed')a(fdmm.consts.UnitType.Train.All,fdmm.consts.UnitType.Static.Train,'Unarmed')a(fdmm.consts.UnitType.Ship.All,fdmm.consts.UnitType.Static.Ship,'Ships')r(fdmm.consts.UnitType.Static)n(fdmm.consts.UnitType.Plane.All,fdmm.consts.UnitType.All)n(fdmm.consts.UnitType.Helicopter.All,fdmm.consts.UnitType.All)n(fdmm.consts.UnitType.Ground.All,fdmm.consts.UnitType.All)n(fdmm.consts.UnitType.Train.All,fdmm.consts.UnitType.All)n(fdmm.consts.UnitType.Ship.All,fdmm.consts.UnitType.All)n(fdmm.consts.UnitType.Static.All,fdmm.consts.UnitType.All)table.concatWith(fdmm.consts.UnitType.Airborne,fdmm.consts.UnitType.Ground.Airborne)table.concatWith(fdmm.consts.UnitType.Amphibious,fdmm.consts.UnitType.Ground.Amphibious)table.concatWith(fdmm.consts.UnitType.CarrierBorne,fdmm.consts.UnitType.Plane.CarrierBorne)table.concatWith(fdmm.consts.UnitType.CarrierBorne,fdmm.consts.UnitType.Helicopter.CarrierBorne)table.concatWith(fdmm.consts.UnitType.HeavyWheeled,fdmm.consts.UnitType.Ground.HeavyWheeled)table.concatWith(fdmm.consts.UnitType.Marines,fdmm.consts.UnitType.Ground.Marines)table.concatWith(fdmm.consts.UnitType.PlayerControllable,fdmm.consts.UnitType.Plane.PlayerControllable)table.concatWith(fdmm.consts.UnitType.PlayerControllable,fdmm.consts.UnitType.Helicopter.PlayerControllable)table.concatWith(fdmm.consts.UnitType.Unavailable,fdmm.consts.UnitType.Plane.Unavailable)table.concatWith(fdmm.consts.UnitType.Unavailable,fdmm.consts.UnitType.Helicopter.Unavailable)table.concatWith(fdmm.consts.UnitType.Unavailable,fdmm.consts.UnitType.Ground.Unavailable)fdmm.utils.ensureReversedDict(fdmm.consts.UnitType.Plane.CarrierBorne)fdmm.utils.ensureReversedDict(fdmm.consts.UnitType.Plane.PlayerControllable)fdmm.utils.ensureReversedDict(fdmm.consts.UnitType.Plane.Unavailable)fdmm.utils.ensureReversedDict(fdmm.consts.UnitType.Helicopter.CarrierBorne)fdmm.utils.ensureReversedDict(fdmm.consts.UnitType.Helicopter.PlayerControllable)fdmm.utils.ensureReversedDict(fdmm.consts.UnitType.Helicopter.Unavailable)fdmm.utils.ensureReversedDict(fdmm.consts.UnitType.Ground.Airborne)fdmm.utils.ensureReversedDict(fdmm.consts.UnitType.Ground.Amphibious)fdmm.utils.ensureReversedDict(fdmm.consts.UnitType.Ground.HeavyWheeled)fdmm.utils.ensureReversedDict(fdmm.consts.UnitType.Ground.Marines)fdmm.utils.ensureReversedDict(fdmm.consts.UnitType.Ground.Unavailable)fdmm.utils.ensureReversedDict(fdmm.consts.UnitType.All)fdmm.utils.ensureReversedDict(fdmm.consts.UnitType.Airborne)fdmm.utils.ensureReversedDict(fdmm.consts.UnitType.Amphibious)fdmm.utils.ensureReversedDict(fdmm.consts.UnitType.CarrierBorne)fdmm.utils.ensureReversedDict(fdmm.consts.UnitType.HeavyWheeled)fdmm.utils.ensureReversedDict(fdmm.consts.UnitType.Marines)fdmm.utils.ensureReversedDict(fdmm.consts.UnitType.PlayerControllable)fdmm.utils.ensureReversedDict(fdmm.consts.UnitType.Unavailable)i(fdmm.consts.UnitType.Plane)i(fdmm.consts.UnitType.Helicopter)i(fdmm.consts.UnitType.Ground)i(fdmm.consts.UnitType.Ship)i(fdmm.consts.UnitType.Train)i(fdmm.consts.UnitType.Static)e(fdmm.consts.UnitType.Plane)e(fdmm.consts.UnitType.Helicopter)e(fdmm.consts.UnitType.Ground)e(fdmm.consts.UnitType.Ship)e(fdmm.consts.UnitType.Train)t(fdmm.consts.UnitType.Plane,fdmm.consts.UnitType.Static)t(fdmm.consts.UnitType.Helicopter,fdmm.consts.UnitType.Static)t(fdmm.consts.UnitType.Ground,fdmm.consts.UnitType.Static)t(fdmm.consts.UnitType.Ship,fdmm.consts.UnitType.Static)t(fdmm.consts.UnitType.Train,fdmm.consts.UnitType.Static)e(fdmm.consts.UnitType.Static)table.concatWith(fdmm.consts.UnitType.Nicknaming,fdmm.consts.UnitType.Plane.Nicknaming)table.concatWith(fdmm.consts.UnitType.Nicknaming,fdmm.consts.UnitType.Helicopter.Nicknaming)table.concatWith(fdmm.consts.UnitType.Nicknaming,fdmm.consts.UnitType.Ground.Nicknaming)table.concatWith(fdmm.consts.UnitType.Nicknaming,fdmm.consts.UnitType.Train.Nicknaming)table.concatWith(fdmm.consts.UnitType.Nicknaming,fdmm.consts.UnitType.Ship.Nicknaming)table.concatWith(fdmm.consts.UnitType.Nicknaming,fdmm.consts.UnitType.Static.Nicknaming)end
...
local e,t=db.getYearsLocal(e,i)n[string.lower(string.trim(i))]={e,t}end
else
n['all']={1900,9999}end
...
function fdmm.unitTypes.crossRefEntries()assert(db,"Missing module: db")assert(dbYears,"Missing module: dbYears")env.info("FDMM: Cross-referencing units...")local function e(r,e)for t,e in ipairs(e)do
local n=e.Name
local e=unit_aliases._rev[n]local i=fdmm.consts.UnitType.All._rev[n]if not i and e then
if type(e)=='string'then
e={e}end
for t,e in ipairs(e)do
i=fdmm.consts.UnitType.All._rev[e]if i then
env.warning("  unitTypeAlias=['"..e.."'] is now unitType=['"..n.."'].")break
end
end
end
if not i then
env.info("  Missing fdmmUnitType for unitType=['"..(n or"<null>").."'] from "..r.."["..t.."].")end
end
end
...
function fdmm.unitTypes.whichAvailableToCountryIn(e,t,i)local n={}local a=string.lower(t)for r,t in pairs(e)do
...
function fdmm.unitTypes.anyAvailableToCountryIn(t,e,n)local i=string.lower(e)for t,e in pairs(t)do
if not string.hasPrefix(t,'_')then
local e=fdmm.unitTypes.getUnitType(e)if not fdmm.unitTypes.isListedUnder(e,fdmm.consts.UnitType.Unavailable)then
...
function fdmm.unitTypes.whichListedUnder(e,t)fdmm.utils.ensureReversedDict(t)local n={}for e,i in pairs(e)do
if not string.hasPrefix(e,'_')then
local r=fdmm.unitTypes.getUnitType(i)if t._rev[r]then
n[e]=i
end
end
end
...
function fdmm.unitTypes.getWTOUnitReportName(e,n)return fdmm.unitTypes.getUnitReportName(fdmm.enums.Alliance.WTO,e,n)end
function fdmm.unitTypes.dumpUnitReportNames()env.info("FDMM: Dumping unit report names...")local function t(e)for i,n in pairs(table.sortedKeysList(e.All))do
if not string.hasPrefix(n,'_')then
local i=e.All[n]local i=fdmm.unitTypes.getUnitType(i)env.info("  ['"..n.."'] = '"..i.."':")env.info("    NATO: "..fdmm.unitTypes.getNATOUnitReportName(i,e))env.info("    WTO : "..fdmm.unitTypes.getWTOUnitReportName(i,e))end
end
end
...
local t=math.floor((e+a)/2)local o=self.ranges[t]if n(o)>r then
a=t
elseif i(o)<r then
e=t+1
else
...
if e-1>=1 and s(i(self.ranges[e-1])+1,self.ranges[e])then
self.ranges[e-1][2]=i(self.ranges[e])table.remove(self.ranges,e)end
...
local t=self.ranges[e]if n(t)>i(t)then
...
function FDMMYearRange:getYearList()local t={}for r,e in ipairs(self.ranges or{})do
if n(e)~=i(e)then
...
function FDMMYearRange:addYearRange(o,a)assert(type(o)=='number',"Invalid parameter: startYear")assert(type(a)=='number',"Invalid parameter: endYear")local r={o,a}local e=table.getn(self.ranges)if e==0 or(o<=n(self.ranges[1])and a>=i(self.ranges[e]))then
...
local o=s(o,l)local s=s(a,m)if o and s then
r={n(l),i(m)}self:_collapseRangesFromTo(e,t)table.insert(self.ranges,e,r)self:_checkCombineNeighborsAt(e)return
elseif not o and not s then
local n=a<n(m)if n then t=t-1 end
self:_collapseRangesFromTo(e,t)table.insert(self.ranges,e,r)self:_checkCombineNeighborsAt(e)return
else
if o then
self:_collapseRangesFromTo(e+1,t)self.ranges[e][2]=i(r)self:_checkCombineNeighborsAt(e)elseif s then
t=t-self:_collapseRangesFromTo(e,t-1)self.ranges[t][1]=n(r)self:_checkCombineNeighborsAt(t)end
return
end
end
...
function FDMMYearRange:removeYearRange(a,r)assert(type(a)=='number',"Invalid parameter: startYear")assert(type(r)=='number',"Invalid parameter: endYear")local e=table.getn(self.ranges)if e==0 or(a<=n(self.ranges[1])and r>=i(self.ranges[e]))then
self.ranges={}return
end
local l={a,r}local e=self:_indexOfRangeAdjToOrCont(a)local m=e and self.ranges[e]or nil
local t=self:_indexOfRangeAdjToOrCont(r)local o=t and self.ranges[t]or nil
local u=s(a,m)local c=s(r,o)local s=a<=n(m)local d=r<n(o)local n=r>=i(o)if s and(d or n)then
if d then t=t-1 end
self:_collapseRangesFromTo(e,t)return
elseif e==t and u and c then
l={r+1,i(m)}self.ranges[e][2]=a-1
table.insert(self.ranges,e+1,l)self:_checkValidRangeAt(e)self:_checkValidRangeAt(e+1)return
else
local o,i=e,t
if not s then o=e+1 end
if not n then i=t-1 end
t=t-self:_collapseRangesFromTo(o,i)if not s and u then
self.ranges[e][2]=a-1
self:_checkValidRangeAt(e)elseif not n and c then
self.ranges[t][1]=r+1
...
local t=nil
for e,n in ipairs(table.sortedKeysList(i))do
assert(type(n)=='number',"Invalid format: startYear")local e=i[n]assert(type(e)~='nil',"Invalid format.")if r==n then return e end
if r<n then return t or e end
t=e
end
return t
end
...
function fdmm.regimentTypes.getObjReportNameIn(n,i)local e=fdmm.regimentTypes._objForYear(n.ReportNaming,i)if string.isNotEmpty(e)then
local t=fdmm.regimentTypes.getObjNameIn(n,i)local n=fdmm.regimentTypes.getUnitHullNumberingIn(n,i)if string.isNotEmpty(t)and string.contains(e,'%N')then
e=e:gsub('%%N',t)end
if string.isNotEmpty(n)and string.contains(e,'%H')then
e=e:gsub('%%H',n)end
return e
end
return fdmm.regimentTypes._objForYear(n.Name,i)end
...
function fdmm.regimentTypes.dumpRegimentYearlyAvailability(o,r,a)env.info("FDMM: Dumping regiment yearly availability for years "..tostring(r).." to "..tostring(a).."...")local function t(i,e,n)env.info("FDMM: "..n.."Regiment: "..fdmm.regimentTypes.getObjReportNameIn(i,e))for t,i in pairs(i.Units or{})do
...
for r,i in pairs(i.Regiments or{})do
t(i,e,n.."  ")end
if i.Reinforcements then
env.info("FDMM: "..n.."  Reinforcements:")for a,r in pairs(i.Reinforcements.Units or{})do
...
for r,i in pairs(i.Reinforcements.Regiments or{})do
t(i,e,n.."    ")end
end
end
end
...
env.info("---FDMM_RegimentTypes End---");end)__DISTILLER:define("FDMM_WeaponTypes",function(e)env.info("---FDMM_WeaponTypes Start---");e('Additions/FDMM_LuaAdditions')fdmm.weaponTypes={}do
fdmm.consts.WeaponType={Missiles={},Rockets={},Guns={},All={},Unavailable={},Available={},NATOReporting={},WTOReporting={},}fdmm.weaponTypes._resKeyFilter={'All','Unavailable','Available','NATOReporting','WTOReporting','ReportNaming','ProperNaming'}function fdmm.weaponTypes._isReservedKey(e)return string.hasPrefix(e,'_')or table.contains(fdmm.weaponTypes._resKeyFilter,e)end
function fdmm.weaponTypes.processEntries()end
function fdmm.weaponTypes.createWeaponTypeAvailability()end
function fdmm.weaponTypes.saveWeaponTypeAvailability()end
function fdmm.weaponTypes.loadWeaponTypeAvailability()end
end
env.info("---FDMM_WeaponTypes End---");end)__DISTILLER:define("Additions/FDMM_MISTAdditions",function(e)env.info("---FDMM_MISTAdditions Start---")assert(mist,'MIST not initialized.')do
...
setmetatable(FDMMRunLoop,{__call=function(e,...)return e.new(...)end,})function FDMMRunLoop.new(n)local e=setmetatable({},FDMMRunLoop)e.name=n
e.isStarted=false
e.isRunning=false
e.shouldExit=false
e.functionList={}return e
end
function FDMMRunLoop:startAsCallback(e)assert(self.isStarted==false,"Cannot start runloop more than once")self.isStarted=true
self.timeInterval=e
timer.scheduleFunction(self._mainByInterval,self,timer.getTime()+self.timeInterval)end
function FDMMRunLoop:startAsCoroutine(e)assert(self.isStarted==false,"Cannot start runloop more than once")self.isStarted=true
self.timeInterval=e
self.coroutine=coroutine.create(function()self._mainByCoroutine(self)end)timer.scheduleFunction(coroutine.resume,self.coroutine,timer.getTime()+self.timeInterval)end
function FDMMRunLoop:stopAsync()self.shouldExit=true
end
function FDMMRunLoop:_resumeLater()self.isRunning=false
if self.shouldExit==false then
if self.coroutine then
timer.scheduleFunction(coroutine.resume,self.coroutine,timer.getTime()+self.timeInterval)coroutine.yield()else
timer.scheduleFunction(self._mainByInterval,self,timer.getTime()+self.timeInterval)end
end
end
function FDMMRunLoop:_mainByInterval()self.isRunning=true
local n,e=pcall(self._main,self)if not n then
env.error("FDMM: Runloop '"..self.name.."' experienced an execution failure in main.")env.error("FDMM:   Error: "..tostring(e))end
self:_resumeLater()end
function FDMMRunLoop:_mainByCoroutine()repeat
self.isRunning=true
local n,e=pcall(self._main,self)if not n then
env.error("FDMM: Runloop '"..self.name.."' experienced an execution failure in main.")env.error("FDMM:   Error: "..tostring(e))end
self:_resumeLater()until self.shouldExit
end
function FDMMRunLoop:_main()self:_runQueuedFuncs()end
function FDMMRunLoop:isMainLoop()return self==fdmm.runLoop.mainLoop
end
function FDMMRunLoop:isActiveLoop()return self.isRunning and coroutine.running()==self.coroutine
end
function FDMMRunLoop:enqueueFuncAsync(e,n)table.insert(self.functionList,{e,n})end
function FDMMRunLoop:enqueueFuncSync(n,e)if self:isActiveLoop()then
n(table.unpack(e))else
local i=true
table.insert(self.functionList,{function()local n,e=pcall(n,table.unpack(e))i=false
return n,e
end,e})while i do
local e=fdmm.runLoop.getActiveLoop()if e and e.coroutine then
e:_resumeLater()else
end
end
end
end
function FDMMRunLoop:_runQueuedFuncs()while table.getn(self.functionList)>0 do
local e=table.remove(self.functionList,1)local e,n=pcall(e[1],table.unpack(e[2]))if not e then
env.error("FDMM: Runloop '"..self.name.."' experienced an execution failure in a queued function.")env.error("FDMM:   Error: "..tostring(n))end
end
end
end
do
function fdmm.runLoop.startRunLoops()fdmm.runLoop.mainLoop=FDMMRunLoop.new("MainLoop")fdmm.runLoop.highPriorityLoop=FDMMRunLoop.new("HighPriorityLoop")fdmm.runLoop.medPriorityLoop=FDMMRunLoop.new("MedPriorityLoop")fdmm.runLoop.lowPriorityLoop=FDMMRunLoop.new("LowPriorityLoop")fdmm.runLoop.mainLoop:startAsCallback(.05)fdmm.runLoop.highPriorityLoop:startAsCoroutine(1)fdmm.runLoop.medPriorityLoop:startAsCoroutine(10)fdmm.runLoop.lowPriorityLoop:startAsCoroutine(60)end
function fdmm.runLoop.getActiveLoop()if fdmm.runLoop.mainLoop:isActiveLoop()then
...
function fdmm.runLoop.stopRunLoops()fdmm.runLoop.mainLoop:stopAsync()fdmm.runLoop.highPriorityLoop:stopAsync()fdmm.runLoop.medPriorityLoop:stopAsync()fdmm.runLoop.lowPriorityLoop:stopAsync()end
end
...
local l=fdmm.cargoRoutes[e][a]local u=mist.getGroupRoute(o,false)local d=fdmm.utils.getFaction(string.notEmptyElse(n.units[1].unitName,n.country))local m=a:upperFirst()local c=table.concatedWith(fdmm.consts.CargoRouteGNPrefix,fdmm.consts.CargoRouteWPRoutePrefix[m])local e=nil
local n={Spawn='spawn',Linkage='linkage',Facility='facility',TBD='_'}local i,r,s=nil,nil,{}function _updateScanner(t,m)if e==n.TBD and t~=nil and t~=n.TBD then
e=t
...
local d,t,i=fdmm.utils.getGroupingComponentsWithSNC(e.name,c,fdmm.consts.CargoRouteWPRouteSuffix)if d==fdmm.consts.CargoRouteGNPrefix[m]then
if i==fdmm.consts.CargoRouteWPRouteSuffix.Egress or
i==fdmm.consts.CargoRouteWPRouteSuffix.Ingress then
_updateScanner(n.TBD,t)table.insert(s,fdmm.utils.rposFromRPoint(e))elseif i==fdmm.consts.CargoRouteWPRouteSuffix.Spawn then
...
_updateScanner(n.Spawn,t)table.insert(s,fdmm.utils.rposFromRPoint(e))elseif not(string.isEmpty(t)and string.isEmpty(i))then
...
_updateScanner(n.Linkage,t)table.insert(s,fdmm.utils.rposFromRPoint(e))elseif not(string.isEmpty(t)and string.isEmpty(i))then
...
_updateScanner(n.Facility,t)table.insert(s,fdmm.utils.rposFromRPoint(e))elseif not(string.isEmpty(t)and string.isEmpty(i))then
...
if e>1 then
local e=mist.utils.makeVec3(a)local e=mist.vec.sub(e,t)local n=mist.vec.mag(e)local a=mist.vec.scalarMult(e,1./n)local e=1
local i=n/e
while i>o do
e=e+1
i=n/e
...
while e>=0 do
local n=mist.vec.scalarMult(a,e*i)local n=mist.vec.add(t,n);COORDINATE:NewFromVec2(mist.utils.makeVec2(n)):Smoke(r)e=e-1
end
end
...
local r,e,s=fdmm.utils.getGroupingComponentsWithSNC(n.name,e,nil)if r==fdmm.consts.TerritoryWPFacilityPrefix.Airbase and string.isNotEmpty(e)then
...
function fdmm.territory.closestTerritoryToPoint(e,n)local t=mist.utils.makeVec2(e)local e=nil
local i=0
local n=n or fdmm.enums.TerritoryType.All
local n=fdmm.territories[n]for r,n in pairs(n)do
local t=mist.utils.get2DDistSqrd(t,n.centerPoint)if e==nil or t<i then
e=n
i=t
end
end
...
setmetatable(FDMMFacility,{__call=function(e,...)return e.new(...)end})function FDMMFacility.new(t,i,n)local e=setmetatable({},FDMMFacility)e.name=t
e.centerPoint=i
e.territoryName=n
return e
end
...
setmetatable(FDMMAirbase,{__index=FDMMFacility,__call=function(e,...)return e.new(...)end})function FDMMAirbase.new(e,i,n)local e=setmetatable(FDMMFacility.new(e,i,n),FDMMAirbase)return e
end
...
setmetatable(FDMMCommandCenter,{__index=FDMMFacility,__call=function(e,...)return e.new(...)end})function FDMMCommandCenter.new(n,i,e)local e=setmetatable(FDMMFacility.new(n,i,e),FDMMCommandCenter)return e
end
...
e('FDMM_Config')e('FDMM_Utils')e('FDMM_UnitTypes')e('FDMM_RegimentTypes')e('FDMM_WeaponTypes')e('Additions/FDMM_LuaAdditions')e('Additions/FDMM_MISTAdditions')e('Additions/FDMM_MOOSEAdditions')e('Utilities/FDMM_RunLoop')e('Utilities/FDMM_YearRange')e('Cargo/FDMM_ResourceUnit')e('Cargo/FDMM_CargoRoute')e('Territory/FDMM_Territory')e('Territory/FDMM_Airbase')e('Territory/FDMM_ArmsPlant')e('Territory/FDMM_CommandCenter')e('Territory/FDMM_FARP')e('Territory/FDMM_OilField')e('Territory/FDMM_Port')e('Territory/FDMM_UnitFactory')do
function fdmm.missionStart()local e="FDMM: Starting "..(fdmm.setup.serverName or"FDMM").."..."env.info(e)trigger.action.outText(e,10)fdmm.config.loadDCSJSONIfAble()fdmm.config.loadConfigOrDefaults()fdmm.config.detectVersions()fdmm.config.loadDCSDBIfAble()fdmm.unitTypes.processEntries()fdmm.regimentTypes.processEntries()fdmm.weaponTypes.processEntries()if db and dbYears and fdmm.config.Config.updatedDCSDetected then
fdmm.unitTypes.createUnitTypeAvailability()fdmm.unitTypes.saveUnitTypeAvailability()fdmm.weaponTypes.createWeaponTypeAvailability()fdmm.weaponTypes.saveWeaponTypeAvailability()else
fdmm.unitTypes.loadUnitTypeAvailability()fdmm.weaponTypes.loadWeaponTypeAvailability()end
...
if fdmm.utils.isRunnableMapKind()then
fdmm.runLoop.startRunLoops()end
