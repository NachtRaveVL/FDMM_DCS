...
do
fdmm.config.Config={_tableVer=1,updatedDCSDetected=false,lastDCSVersionSeen=nil,updatedFDMMDetected=false,lastFDMMVersionSeen=nil}function fdmm.config.runUserSetupScript()env.info("FDDM: Running user setup script...")fdmm.setup={}local e,n=pcall(dofile,fdmm.fullPath..'FDMM_Setup.lua')if not e then
...
env.info("FDDM: Loading DCS DB...")__DCS_VERSION__=fdmm.utils.getDCSVersion()for n=1,3 do
...
function table.concatedWithi(n,i)assert(n or i,'Nonnull parameter: seq1 or seq2')local e={}if n then
...
local function r(i,n)local function r(e,n)for e,i in pairs(e)do
if not fdmm.unitTypes._isReservedKey(e)then
if not n[e]then
n[e]=i
elseif n[e]~=i then
...
r(i.NATOReporting,n.NATOReporting)r(i.WTOReporting,n.WTOReporting)r(i.ReportNaming,n.ReportNaming)r(i.ProperNaming,n.ProperNaming)r(i.Nicknaming,n.Nicknaming)end
...
local e,r=db.getYearsLocal(e,i)n[string.trim(i)]={e,r}end
else
n['ALL']={1900,9999}end
...
function fdmm.unitTypes.whichAvailableToCountryIn(e,a,i)local n={}for r,t in pairs(e)do
...
function fdmm.unitTypes.anyAvailableToCountryIn(e,i,n)for r,e in pairs(e)do
if not string.hasPrefix(r,'_')then
...
function fdmm.unitTypes.anyListedUnder(n,e)fdmm.utils.ensureReversedDict(e)for n,i in pairs(n)do
if not string.hasPrefix(n,'_')then
local n=fdmm.unitTypes.getUnitType(i)if e._rev[n]then
return true
end
end
end
...
local r=math.floor((n+t)/2)local a=self.ranges[r]if e(a)>o then
t=r
elseif i(a)<o then
n=r+1
else
...
function FDMMYearRange:_collapseRangesFromTo(r,e)local i=0
local n=table.getn(self.ranges)while e>=r do
if e>=1 and e<=n then
...
if o then
self:_collapseRangesFromTo(n+1,r)self.ranges[n][2]=i(t)self:_checkCombineNeighborsAt(n)elseif s then
...
local u={a,t}local r=self:_indexOfRangeAdjToOrCont(a)local l=r and self.ranges[r]or nil
local n=self:_indexOfRangeAdjToOrCont(t)local m=n and self.ranges[n]or nil
local d=s(a,l)local s=s(t,m)local o=a<=e(l)local c=t<e(m)local e=t>=i(m)if o and(c or e)then
if c then n=n-1 end
self:_collapseRangesFromTo(r,n)return
elseif r==n and d and s then
u={t+1,i(l)}self.ranges[r][2]=a-1
table.insert(self.ranges,r+1,u)self:_checkValidRangeAt(r)self:_checkValidRangeAt(r+1)return
else
local m,i=r,n
if not o then m=r+1 end
if not e then i=n-1 end
n=n-self:_collapseRangesFromTo(m,i)if not o and d then
self.ranges[r][2]=a-1
self:_checkValidRangeAt(r)elseif not e and s then
self.ranges[n][1]=t+1
self:_checkValidRangeAt(n)end
return
end
...
local r={n,e}local n=self:_indexOfRangeAdjToOrCont(n)local n=n and self.ranges[n]or nil
if n and d(n,r)then
return true
end
...
function fdmm.regimentTypes._objForYear(i,t)if type(i)=='table'then
local r=nil
for e,n in ipairs(table.sortedKeysList(i))do
assert(type(n)=='number',"Invalid format: startYear")local e=i[n]assert(type(e)~='nil',"Invalid format.")if t==n then return e end
if t<n then return r or e end
r=e
end
return r
end
...
function fdmm.regimentTypes.dumpRegimentYearlyAvailability(s,a,o)env.info("FDMM: Dumping regiment yearly availability for years "..tostring(a).." to "..tostring(o).."...")local function r(n,e,i)env.info("FDMM: "..i.."Regiment: "..fdmm.regimentTypes.getObjReportNameIn(n,e))for r,n in pairs(n.Units or{})do
...
env.info("FDMM: "..i.."  Reinforcements:")for a,t in pairs(n.Reinforcements.Units or{})do
if fdmm.regimentTypes.isRegimentUnitAvailableIn(t,e)then
env.info("FDMM: "..i.."    Unit: "..fdmm.regimentTypes.getObjReportNameIn(t,e))end
...
for e=a,o do
env.info("FDMM: Year: "..e)r(s,e,"")end
end
function fdmm.regimentTypes.dumpRegimentActiveLists(t)env.info("FDMM: Dumping regiment year list...")local r=fdmm.setup.equipmentYear
local function i(n,e)env.info("FDMM: "..e.."Regiment: "..fdmm.regimentTypes.getObjReportNameIn(n,r))env.info("FDMM: "..e.."  _Active: "..JSON:encode(n._Active))for i,n in pairs(n.Units or{})do
...
setmetatable(FDMMResourceUnit,{__call=function(e,...)return e.new(...)end,})function FDMMResourceUnit.new(o,a,t,i,r,n)local e=setmetatable({},FDMMResourceUnit)e.fuelRP=o
e.armsRP=a
e.unitRP=t
e.maxFuelRP=i
e.maxArmsRP=r
...
local n={Spawn='spawn',Linkage='linkage',Facility='facility',TBD='_'}local i,t,s=nil,nil,{}function _updateScanner(r,m)if e==n.TBD and r~=nil and r~=n.TBD then
e=r
...
elseif e~=n.TBD then
env.error('Cargo '..o.." routing group '"..a.."' encountered unrecognized scan mode '"..(r or'<nil>').."'.")end
i,t,s=nil,nil,{}end
e=r
i=m
end
...
local d,r,i=fdmm.utils.getGroupingComponentsWithSNC(e.name,c,fdmm.consts.CargoRouteWPRouteSuffix)if d==fdmm.consts.CargoRouteGNPrefix[m]then
...
fdmm.facilities.all[e.name]=e
r:addFacility(e)elseif not(string.isEmpty(e)and string.isEmpty(o))then
...
local n=mist.utils.get2DDistSqrd(t,i.centerPoint)if e==nil or n<r then
e=i
r=n
end
end
...
setmetatable(FDMMFARP,{__index=FDMMFacility,__call=function(e,...)return e.new(...)end})function FDMMFARP.new(i,e,n)local e=setmetatable(FDMMFacility.new(i,e,n),FDMMFARP)return e
end
...
fdmm.territory.buildFacilities()fdmm.config.saveConfig()if fdmm.utils.isDevRunMode()then
